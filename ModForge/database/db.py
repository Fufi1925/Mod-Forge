...
class Database:
    ...
    async def ensure_indexes(self) -> None:
        try:
            ...
            await self.message_archive.create_index(
                [("guild_id", ASCENDING), ("user_id", ASCENDING), ("timestamp", DESCENDING)], name="archive_user_lookup")
    ...
    async def aupdate_case_reason(self, guild_id: int, case_id: int, new_reason: str, mod_id: int) -> bool:
        try:
            ...
            if not existing:
                return False
            history_entry = {
                "old_reason": existing.get("reason", ""), "changed_by": mod_id,
    ...
    async def aattach_messages_to_case(self, guild_id: int, case_id: int,
    ...
    async def aappend_message_edit(self, guild_id: int, message_id: int,
