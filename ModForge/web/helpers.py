...
def _bot_stats():
    """Gibt (guild_count, member_count, uptime_seconds, latency_ms) zurück."""
    gc = mc = up = lat = 0
    try:
        ...
        if BOT_REF is not None:
            ...
            up = get_uptime(BOT_REF.start_time)  # korrigiert: direkter Aufruf der Modul-Funktion
...
def _get_guild_config(guild_id: str) -> dict:
    """Lädt die Konfiguration einer Guild aus der Datenbank (falls verfügbar)."""
    from bot.bot import BOT_REF
    if BOT_REF is not None:
        cfg = _run_async(BOT_REF.db.aget_config(int(guild_id)))  # korrigiert: aget_config statt load_config
        if cfg:
            return cfg
...
def _build_module_form(section: str, cfg: dict, guild_id: str) -> str:
    """Erzeugt das HTML-Formular für ein einzelnes Modul (Security, AutoMod, Verifikation etc.)."""
    mod_map = {
    ...
    def save_fn() -> str:
        ...
        if section == "antispam":
            fields.update(max_messages="getInt('mx')", interval="getInt('iv')",
                          caps_pct="getInt('cp')", emoji_max="getInt('em')")
        elif section == "antinuke":
            fields.update(ban_threshold="getInt('bt')",
                          channel_delete_threshold="getInt('ct')",
                          window_seconds="getInt('ws')")
        elif section == "antiraid":
            fields.update(join_threshold="getInt('jt')",
                          window_seconds="getInt('ws')",
                          min_account_age_days="getInt('ma')")
        elif section == "antimention":
            fields.update(max_mentions="getInt('mm')",
                          timeout_seconds="getInt('td')")
        obj = ",".join(f'"{k}":{v}' for k, v in fields.items())
        return f"saveMod('{mod}',{{{obj}}})"
    ...
    <div class="settings-row">
        ...
        <label class="toggle"><input type="checkbox" id="en" {"checked" if enabled else ""}>
...
def _build_welcome_content(cfg: dict, guild_id: str) -> str:
    """Erzeugt das HTML für Welcome- und Leave-Einstellungen."""
    wc = cfg.get("welcome", {})
    lc = cfg.get("leave", {})
...
<div class="settings-panel">
    ...
    <div class="settings-row">
        ...
        <label class="toggle"><input type="checkbox" id="wc-en" {"checked" if wc.get('enabled') else ""}>
        <div class="toggle-track"></div><div class="toggle-thumb"></div></label>
    </div>
    <div class="embed-builder" style="margin-top:16px">
        <div class="embed-form">
            ...
            <div class="settings-row" style="padding:8px 0">
                ...
                <label class="toggle"><input type="checkbox" id="wc-thumb" {"checked" if wc.get('embed_thumbnail',True) else ""}>
                <div class="toggle-track"></div><div class="toggle-thumb"></div></label>
            </div>
            <div style="border-top:1px solid var(--border);margin:12px 0;padding-top:12px">
                <div class="settings-row" style="padding:0;margin-bottom:10px">
                    ...
                    <label class="toggle"><input type="checkbox" id="wc-dm" {"checked" if wc.get('dm_enabled') else ""}>
                    <div class="toggle-track"></div><div class="toggle-thumb"></div></label>
                </div>
...
<div class="settings-panel">
    ...
    <div class="settings-row">
        <div><div class="settings-row-label">Aktiviert</div></div>
        <label class="toggle"><input type="checkbox" id="lv-en" {"checked" if lc.get('enabled') else ""}>
        <div class="toggle-track"></div><div class="toggle-thumb"></div></label>
    </div>
