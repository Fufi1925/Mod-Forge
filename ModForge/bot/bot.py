...
class Tracker:
    def __init__(self) -> None:
        self.spam_tracker: Dict[int, Dict[int, deque]] = defaultdict(
            lambda: defaultdict(deque)
        )
        self.dup_tracker: Dict[int, Dict[int, List]] = defaultdict(
            lambda: defaultdict(list)
        )
        self.nuke_tracker: Dict[int, Dict[int, deque]] = defaultdict(
            lambda: defaultdict(deque)
        )
        self.raid_tracker: Dict[int, deque] = defaultdict(deque)
        self.new_account_tracker: Dict[int, deque] = defaultdict(deque)
        self.mention_tracker: Dict[int, Dict[int, deque]] = defaultdict(
            lambda: defaultdict(deque)
        )
...
class CaptchaModal(discord.ui.Modal, title="Verifizierung"):
    answer = discord.ui.TextInput(
        label="Gib den Code aus dem Bild ein",
        placeholder="CODE123",
        min_length=4,
        max_length=8,
    )
    ...
    def __init__(
        self, correct_code: str, bot_ref, add_roles: List[int], remove_roles: List[int]
    ) -> None:
        super().__init__()
        ...
        if self.answer.value.upper() == self.correct_code:
            try:
                for r_id in self.add_roles:
                    ...
                    if role:
                        await interaction.user.add_roles(
                            role, reason="Verifizierung erfolgreich"
                        )
                ...
                await interaction.response.send_message(
                    f"{E.OK} Verifizierung erfolgreich!", ephemeral=True
                )
                await self.bot.log_action(
                    interaction.guild,
                    f"{E.VERIFY} Verifiziert",
                ...
                await interaction.response.send_message(
                    f"{E.FAIL} Mir fehlen Rechte.", ephemeral=True
                )
            except discord.HTTPException as ex:
                log.warning(f"Captcha-Rollenfehler: {ex}")
                await interaction.response.send_message(
                    f"{E.FAIL} Discord-Fehler.", ephemeral=True
                )
        else:
            await interaction.response.send_message(
                f"{E.FAIL} Falscher Code!", ephemeral=True
            )


class CaptchaEntryView(discord.ui.View):
    def __init__(
        self, code: str, bot_ref, add_roles: List[int], remove_roles: List[int]
    ) -> None:
    ...
    async def enter_code(
        ...
        await interaction.response.send_modal(
            CaptchaModal(self.code, self.bot, self.add_roles, self.remove_roles)
        )
        ...
        if cfg["mode"] == "one_click":
            try:
                for r_id in add_roles:
                    ...
                    if role:
                        await interaction.user.add_roles(
                            role, reason="One-Click-Verifizierung"
                        )
            ...
            await interaction.response.send_message(
                "Bitte gib den Code aus diesem Bild ein:",
                file=file,
                ephemeral=True,
...
class TicketView(discord.ui.View):
    ...
    @discord.ui.button(
        ...
        custom_id="modforge:open_ticket",
        emoji=E.TICKET,
    )
    async def open_ticket(
        ...
        category = (
            interaction.guild.get_channel(cfg["category_id"])
...
class TicketCloseView(discord.ui.View):
    ...
    @discord.ui.button(
        ...
        custom_id="modforge:close_ticket",
        emoji=E.LOCK,
    )
    async def close_ticket(
        ...
        await interaction.response.send_message(
            "Das Ticket wird in 5 Sekunden geschlossen..."
        )
        await self.bot.log_action(
            interaction.guild,
            f"{E.LOCK} Ticket geschlossen",
        ...
        try:
            await interaction.channel.delete(
                reason=f"Ticket geschlossen durch {interaction.user}"
            )
        except discord.Forbidden:
            pass
...
class AppealReasonModal(discord.ui.Modal):
    ...
    async def on_submit(self, interaction: discord.Interaction) -> None:
        ...
        if self.action == "unban":
            try:
                await guild.unban(
                    ...
                    reason=f"Appeal akzeptiert: {self.reason.value}",
                )
        ...
        await self.bot_ref.log_action(
            guild,
            result_title,
            f"Appeal von <@{user_id}> bearbeitet.",
            color,
            module="appeal",
        )
...
class AppealActionView(discord.ui.View):
        ...
        if interaction.message and interaction.message.embeds:
            ...
            for part in footer.split("|"):
                ...
                if part.startswith("UserID:"):
                    try:
                        return {
                            "user_id": int(part.replace("UserID:", "").strip()),
    ...
    async def decline(
        ...
        try:
            ...
            await user.send(
                f"{E.FAIL} **Dein Entbannungs-Antrag wurde abgelehnt.** Du bleibst vom Server **{guild.name}** gebannt."
            )
        except (discord.NotFound, discord.Forbidden, discord.HTTPException):
            pass
        await interaction.client.log_action(
            guild,
            f"{E.FAIL} Appeal abgelehnt",
            f"Appeal von <@{user_id}> abgelehnt.",
            COLOR_DANGER,
            module="appeal",
        )
    ...
    async def unban(
        ...
        try:
            user = await interaction.client.fetch_user(user_id)
            await user.send(
                f"{E.OK} **Dein Entbannungs-Antrag wurde angenommen!** Willkommen zurück auf **{guild.name}**."
            )
        except (discord.NotFound, discord.Forbidden, discord.HTTPException):
            pass
        await interaction.client.log_action(
            guild,
            f"{E.OK} Appeal akzeptiert",
            f"<@{user_id}> entbannt von {interaction.user.mention}",
            COLOR_SUCCESS,
            module="appeal",
        )

    @discord.ui.button(
        label="Ablehnen + Grund",
        style=discord.ButtonStyle.secondary,
        custom_id="appeal:decline_reason",
        emoji="📝",
    )
    async def decline_reason(
        self, interaction: discord.Interaction, button: discord.ui.Button
    ) -> None:
        if not interaction.user.guild_permissions.ban_members:
            await interaction.response.send_message(
        ...
        data = self._get_appeal_data(interaction)
        await interaction.response.send_modal(
            AppealReasonModal("decline", data, interaction.client)
        )

    @discord.ui.button(
        label="Entbannen + Notiz",
        style=discord.ButtonStyle.primary,
        custom_id="appeal:unban_reason",
        emoji=E.TEXT,
    )
    async def unban_reason(
        self, interaction: discord.Interaction, button: discord.ui.Button
    ) -> None:
        if not interaction.user.guild_permissions.ban_members:
            await interaction.response.send_message(
        ...
        data = self._get_appeal_data(interaction)
        await interaction.response.send_modal(
            AppealReasonModal("unban", data, interaction.client)
        )
...
class SetupModuleSelect(discord.ui.Select):
    def __init__(self) -> None:
        options = [
            ...
            discord.SelectOption(
                label="Anti-Mention", value="anti_mention", emoji=E.MENTION
            ),
            discord.SelectOption(label="AutoMod", value="automod", emoji=E.AUTOMOD),
            discord.SelectOption(
                label="Anti-Ghost-Ping", value="anti_ghost_ping", emoji=E.GHOST
            ),
            discord.SelectOption(label="Anti-Scam", value="anti_scam", emoji=E.SCAM),
            discord.SelectOption(
                label="Anti-URL-Shortener", value="anti_url_shortener", emoji=E.URLSHORT
            ),
        ]
    ...
    async def callback(self, interaction: discord.Interaction) -> None:
        ...
        embed = create_embed(
            ...
            f"Aktuelle Einstellungen für **{module}**",
            COLOR_PRIMARY,
            fields,
        )
...
class SetupView(discord.ui.View):
    ...
    @discord.ui.button(
        label="Log-Kanal setzen", style=discord.ButtonStyle.primary, emoji=E.CHANNEL
    )
    ...
    @discord.ui.button(
        label="Prefix ändern", style=discord.ButtonStyle.secondary, emoji=E.PREFIX
    )
...
class LogChannelModal(discord.ui.Modal, title="Log-Kanal setzen"):
    channel_id = discord.ui.TextInput(
        label="Kanal-ID", placeholder="z.B. 1234567890123456789", required=True
    )
    ...
    async def on_submit(self, interaction: discord.Interaction) -> None:
        try:
            ...
            await interaction.response.send_message(
                f"{E.FAIL} Ungültige Kanal-ID.", ephemeral=True
            )
            return
        cfg = interaction.client.db.get_config(interaction.guild.id)
        cfg["log_channel"] = cid
        ...
        await interaction.response.send_message(
            embed=create_embed(f"{E.OK} Log-Kanal gesetzt", f"<#{cid}>", COLOR_SUCCESS),
            ephemeral=True,
        )
...
class PrefixModal(discord.ui.Modal, title="Prefix ändern"):
    prefix = discord.ui.TextInput(
        label="Neuer Prefix", placeholder="z.B. !", max_length=5, required=True
    )
    ...
    async def on_submit(self, interaction: discord.Interaction) -> None:
        ...
        await interaction.response.send_message(
            embed=create_embed(
                f"{E.OK} Prefix geändert", f"`{self.prefix.value}`", COLOR_SUCCESS
            ),
            ephemeral=True,
        )
...
class WarnSetupModal(discord.ui.Modal, title="Warn-Schwellen konfigurieren"):
    threshold_3 = discord.ui.TextInput(
        label="Strafe bei 3 Verwarnungen",
        placeholder="warn / timeout / kick / ban",
        default="timeout",
        required=True,
    )
    threshold_5 = discord.ui.TextInput(
        label="Strafe bei 5 Verwarnungen",
        placeholder="warn / timeout / kick / ban",
        default="kick",
        required=True,
    )
    threshold_7 = discord.ui.TextInput(
        label="Strafe bei 7 Verwarnungen",
        placeholder="warn / timeout / kick / ban",
        default="ban",
        required=True,
    )
    ...
    async def on_submit(self, interaction: discord.Interaction) -> None:
        values = {
            "3": self.threshold_3.value,
            "5": self.threshold_5.value,
            "7": self.threshold_7.value,
        }
        for v in values.values():
            if v not in VALID_PUNISHMENTS:
                await interaction.response.send_message(
                    embed=create_embed(
                        f"{E.FAIL} Fehler",
                        f"Ungültige Strafe. Erlaubt: {', '.join(VALID_PUNISHMENTS)}",
                        COLOR_DANGER,
                    ),
                    ephemeral=True,
                )
        ...
        embed = create_embed(
            f"{E.OK} Warn-Schwellen gesetzt",
            "Automatische Strafen wurden konfiguriert.",
            COLOR_SUCCESS,
            fields,
        )
...
class HelpCategorySelect(discord.ui.Select):
    def __init__(self) -> None:
        options = [
            ...
            discord.SelectOption(
                label="Logs (pro Modul)", value="logs", emoji=E.CHANNEL
            ),
...
class ModForge(commands.Bot):
    def __init__(self) -> None:
        ...
        super().__init__(
            command_prefix=self._get_prefix, intents=intents, help_command=None
        )
        self.db = Database()
        ...
        self.status_rotation = [
            ...
            (
                ...
                "👀 Watching {member_count} Members | /help",
                10,
            ),
        ]
    ...
    async def setup_hook(self) -> None:
        ...
        from bot.config import ACTIVITY
        ACTIVITY.set_collection(self.db.db["activities"])
        await self.db.db["activities"].create_index("ts", expireAfterSeconds=86400*7)
    ...
    async def on_ready(self) -> None:
        ...
        ACTIVITY.push(
            "ready",
            f"Bot online als {self.user} – {len(self.guilds)} Guilds, {sum(g.member_count or 0 for g in self.guilds)} Member.",
        )
    ...
    async def _warmup_caches(self) -> None:
        """Lädt alle Guild-Configs asynchron."""
        loaded = 0
        failed = 0
        ...
        try:
            ...
            if fields:
                for fname, fvalue, finline in fields:
                    embed.add_field(
                        name=str(fname), value=str(fvalue), inline=bool(finline)
                    )
            if thumb:
            ...
            if user and hasattr(user, "name"):
                embed.set_author(
                    ...
                    icon_url=thumb or discord.utils.MISSING,
                )
        except Exception as e:
    ...
    def _resolve_log_channel(self, cfg: dict, module: str):
        ...
        if module in log_channels:
            ...
            if str(ch_id) == "0":
                return None  # Explizit deaktiviert
            if ch_id:
                return int(ch_id)
    ...
    async def _send_webhook(
        self, channel, embed, module, wl_cfg, cfg, guild_id
    ) -> bool:
        ...
        try:
            ...
            if _wh_url:
                try:
                    ...
                    # Nutze die bestehende aiohttp Session vom Bot (Client) anstatt ständig neue Sockets zu öffnen (verhindert Port Exhaustion!)
        ...
        for attempt in range(max_retries):
            try:
                ...
                if attempt < max_retries - 1:
                    wait = 2**attempt
                    log.warning(
                        f"HTTP-Fehler beim Loggen ({attempt + 1}/{max_retries}): {ex}"
                    )
                    await asyncio.sleep(wait)
                else:
                    log.error(
                        f"HTTP-Fehler beim Loggen nach {max_retries} Versuchen: {ex}"
                    )
            except Exception as e:
                if attempt < max_retries - 1:
                    wait = 2**attempt
    ...
    async def punish(
        self, member, punishment, reason, duration=60, moderator_id=None
    ) -> Optional[int]:
        guild = member.guild
        bot_member = guild.me
        ...
        try:
            if punishment == "warn":
                ...
                dm_e = create_embed(
                    ...
                    f"{E.RED} **Achtung:** Bei weiteren Verwarnungen drohen härtere Strafen!"
                    if count >= 2
                    else "",
                    COLOR_WARNING,
                ...
                dm_e.set_footer(
                    text=f"{guild.name} · ModForge Security", icon_url=FOOTER_ICON
                )
                    ...
                    serializable = [
                        {
                            ...
                            "edits": m.get("edits", []),
                            "timestamp": (
                                m.get("timestamp") or discord.utils.utcnow()
                            ).isoformat()
                            + "Z",
                        }
                        for m in pre_messages
                    ]
                    await self.db.aattach_messages_to_case(
                        guild.id, case_id, serializable
                    )
                ACTIVITY.push(
                    "case",
                    f"Case #{case_id} (ban) – {member} | {len(pre_messages)} archivierte Nachrichten",
                    guild_id=guild.id,
                    guild_name=guild.name,
                    user_id=member.id,
            ...
            try:
                if warn_count == int(threshold_str):
                    await self.punish(
                        member,
                        action,
                        f"Automatisch: {warn_count} Verwarnungen erreicht",
                    )
                    await self.log_action(
                        member.guild,
        ...
        for entry in due:
            ...
            try:
                if entry["action"] == "tempban":
                    try:
                        await guild.unban(
                            discord.Object(id=entry["user_id"]),
                    ...
                    await self.log_action(
                        guild,
                        f"{E.OK} Tempban abgelaufen",
                        f"<@{entry['user_id']}> entbannt.",
                        COLOR_SUCCESS,
                        module="moderation",
                    )
                    ...
                    await self.log_action(
                        guild,
                        f"{E.UNMUTE} Tempmute abgelaufen",
                        f"<@{entry['user_id']}> entmutet.",
                        COLOR_SUCCESS,
                        module="moderation",
                    )
            finally:
        ...
        for entry in active:
            ...
            if end_time and end_time <= now:
                try:
                    await member.timeout(
                        None, reason="Persistent-Mute abgelaufen (Recovery)"
                    )
            ...
            if end_time:
                try:
                    await member.timeout(
                        end_time,
                        reason="Persistent-Mute Wiederherstellung nach Neustart",
                    )
                except (discord.Forbidden, discord.NotFound, discord.HTTPException):
                    pass
...
async def on_member_join(member):
    ...
    ACTIVITY.push(
        "join",
        f"{member} ist **{guild.name}** beigetreten ({guild.member_count} Member).",
        guild_id=guild.id,
        guild_name=guild.name,
        user_id=member.id,
        user_name=str(member),
    ...
    if vpn_cfg.get("enabled") and not member.bot:
        ...
        if str(member.id) not in [str(w) for w in vpn_wl]:
            ...
            account_age_hours = (
                discord.utils.utcnow() - member.created_at.replace(tzinfo=None)
            ).total_seconds() / 3600
            suspicious_vpn = (
                account_age_hours < 24
                and not member.avatar
                and (
                    member.name.startswith("user")
                    or len(member.name) < 4
                    or any(c.isdigit() for c in member.name[-4:])
                )
            )
            if suspicious_vpn:
                ...
                try:
                    if action == "ban":
                        await member.ban(
                            reason="Anti-VPN: Verdächtiger Account (VPN/Proxy-Heuristik)"
                        )
                    elif action == "kick":
                        await member.kick(
                            reason="Anti-VPN: Verdächtiger Account (VPN/Proxy-Heuristik)"
                    ...
                    await bot.log_action(
                        guild,
                        f"{E.ALERT}️ Anti-VPN",
                        f"Kann {member.mention} nicht {action}en (fehlende Rechte).",
                        COLOR_WARNING,
                        module="antiraid",
                    )
    ...
    if cfg.get("raidmode_active") and not member.bot:
        try:
            ...
            await safe_dm(
                member, dm_embed, cooldown_key=f"raidmode_dm:{guild.id}:{member.id}"
            )
        except Exception:
            pass
    ...
    if sec_level >= 2:
        ...
        if account_age < 5:
            try:
                await member.kick(
                    reason="Sicherheitsstufe 2: Account zu jung (< 5 Tage)"
                )
                await bot.log_action(
                    guild,
                    f"{E.SHIELD} Sicherheitsstufe 2: Kick",
                    f"{member.mention} (Account-Alter: {account_age} Tage).",
                    COLOR_DANGER,
                    user=member,
                    module="antiraid",
                )
    ...
    if sec_level >= 3:
        try:
            ...
            await bot.log_action(
                guild,
                f"{E.SHIELD} Sicherheitsstufe 3: BLACKOUT BAN",
                f"{member.mention} sofort gebannt.",
                COLOR_DANGER,
                user=member,
                module="antiraid",
            )
    ...
    if raid_cfg.get("enabled"):
        ...
        if account_age < min_age_days and raid_cfg.get("auto_kick"):
            try:
                await member.kick(
                    reason=f"Anti-Raid: Account zu jung ({account_age} Tage)"
                )
                await bot.log_action(
                    guild,
                    f"{E.NEWACC} Neuer Account gekickt",
                    f"{member.mention} (Account-Alter: {account_age} Tage).",
                    COLOR_DANGER,
        ...
        if account_age < min_age_days:
            ...
            if len(ndq) >= new_acc_thresh:
                await bot.log_action(
                    guild,
                    f"{E.RAID} Neue-Account-Cluster",
                    f"{len(ndq)} junge Accounts in {new_acc_window}s.",
                    COLOR_DANGER,
                    module="antiraid",
                )
        if raid_cfg.get("suspicious_name_check") and SUSPICIOUS_NAME_REGEX.match(
            member.name
        ):
            await bot.log_action(
                guild,
                f"{E.RAID} Verdächtiger Username",
        ...
        if len(dq) >= raid_cfg.get("join_threshold", 10):
            ...
            await bot.log_action(
                guild,
                f"{E.NUKE} RAID ERKANNT – LOCKDOWN",
                "Zu viele Beitritte!",
                COLOR_DANGER,
                module="antiraid",
            )
    ...
    if user_id in appeal_sessions:
        ...
        if content.lower() == "!abbruch":
            ...
            await message.author.send(
                embed=create_embed(
                    "🚫 Appeal abgebrochen",
                    "Du hast den Entbannungs-Antrag abgebrochen.",
                    COLOR_WARNING,
                )
            )
            return True
        ...
        if next_step < len(APPEAL_QUESTIONS):
            await message.author.send(
                embed=create_embed(
                    f"{E.OK} Antwort gespeichert",
                    APPEAL_QUESTIONS[next_step],
                    COLOR_INFO,
                )
            )
        else:
...
async def _submit_appeal(user: discord.User, session: dict) -> None:
    ...
    embed.set_footer(
        text=f"Powered by BotForge 🔒 | UserID: {user.id}", icon_url=FOOTER_ICON
    )

    view = AppealActionView(
        bot_ref=bot, appeal_data={"user_id": user.id, "guild_id": guild_id}
    )
...
async def _deactivate_lockdown(guild: discord.Guild) -> None:
    ...
    for channel in guild.text_channels:
        try:
            await channel.set_permissions(
                guild.default_role, send_messages=None, reason="Lockdown aufgehoben"
            )
...
async def _notify_owner_nuke(
    ...
    if executor:
        ...
        roles = (
            ", ".join(r.name for r in executor.roles if r.name != "@everyone")
            or "Keine"
        )
    ...
    embed.add_field(
        name=f"{E.LAW}️  Strafe", value=(f"```fix\n{punishment}\n```"), inline=True
    )
    ...
    await safe_dm(
        owner, embed, cooldown_key=f"nuke_alert:{guild.id}", cooldown_seconds=60
    )
...
async def _nuke_check(guild: discord.Guild, executor_id: int, action: str) -> None:
    ...
    if len(dq) >= nuke_cfg.get("threshold", 5):
        ...
        if can_act:
            await bot.punish(
                executor, punishment, (f"Anti-Nuke: Verdächtige Aktivität ({action})")
            )
        else:
            ...
            if owner:
                ...
                alert_embed.add_field(
                    name="🔍 Erkannte Aktion", value=f"```{action}```", inline=False
                )
                await safe_dm(
                    owner,
                    alert_embed,
    ...
    try:
        async for entry in guild.audit_logs(limit=5, action=action):
            if target_id is None or (
                entry.target and getattr(entry.target, "id", None) == target_id
            ):
                return entry.user
...
async def on_message(message: discord.Message) -> None:
    ...
    if spam_cfg.get("enabled"):
        ...
        if len(dq) > spam_cfg.get("msg_limit", 5):
            ...
            await bot.punish(
                member,
                spam_cfg.get("punishment", "timeout"),
                "Anti-Spam: Nachrichtenflut",
                spam_cfg.get("timeout_duration", 30),
            )
            await bot.log_action(
                guild,
                f"{E.SPAM} Anti-Spam ausgelöst",
        ...
        if len(content) > 10:
            caps_ratio = (
                sum(1 for c in content if c.isupper()) / max(len(content), 1) * 100
            )
    ...
    if automod_cfg.get("enabled"):
        ...
        for word in automod_cfg.get("bad_words", []):
            try:
                if re.search(rf"\b{re.escape(word)}\b", content, re.IGNORECASE):
                    ...
                    await bot.punish(
                        member,
                        automod_cfg.get("punishment", "warn"),
                        f"AutoMod: Verbotenes Wort '{word}'",
                    )
                    await bot.log_action(
                        guild,
                        f"{E.BADWORD} Verbotenes Wort erkannt",
        ...
        for pattern in automod_cfg.get("regex_rules", []):
            try:
                if re.search(pattern, content):
                    ...
                    await bot.log_action(
                        guild,
                        f"{E.BADWORD} AutoMod Regex",
                        f"{member.mention} – Pattern erkannt.",
                        COLOR_DANGER,
                        [("Pattern", f"`{pattern[:80]}`", False)],
                        user=member,
                        module="automod",
                    )
                    await bot.process_commands(message)
        ...
        if automod_cfg.get("invite_filter") and INVITE_REGEX.search(content):
            ...
            await bot.log_action(
                guild,
                f"{E.BADWORD} Invite-Link erkannt",
        ...
        if automod_cfg.get("link_filter"):
            ...
            for url in urls:
                ...
                if not any(domain.endswith(d.lower()) for d in allowed):
                    ...
                    await bot.log_action(
                        guild,
                        f"{E.BADWORD} Link-Filter ausgelöst",
        ...
        if automod_cfg.get("zalgo_filter") and ZALGO_REGEX.search(content):
            ...
            await bot.log_action(
                guild,
                f"{E.BADWORD} Zalgo-Text erkannt",
        ...
        if automod_cfg.get("unicode_abuse"):
            suspicious = sum(
                1 for c in content if unicodedata.category(c) in ("Cf", "Cs", "Co")
            )
            if suspicious > 5:
                try:
                    await message.delete()
                ...
                await bot.log_action(
                    guild,
                    f"{E.BADWORD} Unicode-Missbrauch erkannt",
    ...
    if scam_cfg.get("enabled"):
        for domain in SCAM_DOMAINS:
            if domain.lower() in message.content.lower():
                ...
                await bot.punish(
                    member,
                    scam_cfg.get("punishment", "ban"),
                    f"Anti-Scam: Scam-Domain '{domain}'",
                )
                await bot.log_action(
                    guild,
                    f"{E.BADWORD} Scam-Link erkannt",
    ...
    if short_cfg.get("enabled"):
        for shortener in URL_SHORTENERS:
            if shortener.lower() in message.content.lower():
                ...
                await bot.punish(
                    member,
                    short_cfg.get("punishment", "warn"),
                    f"Anti-URL-Shortener: '{shortener}'",
                )
                await bot.log_action(
                    guild,
                    f"{E.URLSHORT} URL-Shortener erkannt",
                    f"{member.mention} hat einen URL-Shortener verwendet.",
                    COLOR_WARNING,
                    user=member,
                    module="antishortener",
                )
                await bot.process_commands(message)
                return
    ...
    if mention_cfg.get("enabled") and (message.mentions or message.role_mentions):
        ...
        if len(dq) > mention_cfg.get("mention_limit", 5):
            ...
            await bot.punish(
                member,
                mention_cfg.get("punishment", "timeout"),
                "Anti-Mention: Mention-Spam",
                mention_cfg.get("timeout_duration", 60),
            )
            await bot.log_action(
                guild,
                f"{E.MENTION} Mention-Spam erkannt",
...
async def on_message_delete(message: discord.Message) -> None:
    ...
    if ghost_cfg.get("enabled") and message.id in bot.tracker.ghost_tracker.get(
        guild.id, {}
    ):
        ...
        await bot.log_action(
            ...
            [
                ...
                (
                    "Inhalt",
                    f"||{message.content[:200]}||" if message.content else "*(leer)*",
                    False,
                ),
            ],
            user=author,
...
async def on_voice_state_update(
        ...
        try:
            ...
            if tv.get("enabled") and tv.get("channel_id"):
                if after.channel and after.channel.id == tv["channel_id"]:
                    cat = (
                        guild.get_channel(tv.get("category_id"))
                    ...
                    overwrites = {
                        ...
                        guild.me: discord.PermissionOverwrite(
                            read_messages=True, connect=True, manage_channels=True
                        ),
                    }
                ...
                if before.channel and before.channel != after.channel:
                    if (
                        before.channel.name.startswith(f"{E.VOICE} ")
                    ...
                    try:
                        ...
                        async for entry in guild.audit_logs(
                            ...
                            if (
                                ...
                                and (now - entry.created_at).total_seconds() < 7
                            ):
                                actor = entry.user
                                break
                    except Exception:
        ...
        try:
            ...
            if not member.bot:
                try:
                    ...
                    pass
            title = (
                f"{E.MUTE} Server Mute" if after.mute else f"{E.UNMUTE} Server Unmute"
            )
            status = (
                "Server-stummgeschaltet 🔇"
                if after.mute
                else "Server-entstummgeschaltet"
            )
            color = COLOR_WARNING if after.mute else COLOR_SUCCESS
            await bot.log_action(
                guild,
                title,
                f"**{member.mention}** wurde **{status}**{mod_info}",
                color,
                user=member,
                module="voice",
            )
    ...
    if before.self_stream != after.self_stream:
        try:
            ...
            desc = (
                f"**{member.mention}** streamt in {after.channel.mention if after.channel else 'Voice'}"
                if after.self_stream
                else f"**{member.mention}** hat den Stream beendet."
            )
            await bot.log_action(
                guild,
                title,
                desc,
                COLOR_PRIMARY if after.self_stream else COLOR_INFO,
                user=member,
                module="voice",
            )
        except Exception:
    ...
    if before.self_video != after.self_video:
        try:
            ...
            await bot.log_action(
                guild,
                title,
                f"**{member.mention}** Kamera {'eingeschaltet' if after.self_video else 'ausgeschaltet'}",
                COLOR_PRIMARY if after.self_video else COLOR_INFO,
                user=member,
                module="voice",
            )
        except Exception:
...
async def on_member_update(before: discord.Member, after: discord.Member) -> None:
    ...
    if before.roles != after.roles:
        ...
        if added:
            await bot.log_action(
                guild,
                f"{E.ROLE} Rolle gegeben",
                f"{after.mention} wurde eine Rolle gegeben.",
                COLOR_SUCCESS,
                [
                    ("Rollen", ", ".join(added), False),
                    (
                        "Moderator",
                        mod.mention if hasattr(mod, "mention") else str(mod),
                        True,
                    ),
                ],
                user=after,
        ...
        if removed:
            await bot.log_action(
                guild,
                f"{E.ROLE} Rolle entfernt",
                f"{after.mention} wurde eine Rolle entzogen.",
                COLOR_DANGER,
                [
    ...
    if before.nick != after.nick:
        ...
        await bot.log_action(
            guild,
            f"{E.NICK} Nickname geändert",
            f"{after.mention} hat einen neuen Nicknamen.",
            COLOR_INFO,
            [
                ...
                (
                    "Geändert von",
                    mod_nick.mention if hasattr(mod_nick, "mention") else str(mod_nick),
                    True,
                ),
            ],
            user=after,
        ...
        if after_timeout and (not before_timeout or after_timeout > before_timeout):
            ...
            try:
                ...
                if total_seconds > 86400:
                    remaining = (
                        f"{total_seconds // 86400}d {(total_seconds % 86400) // 3600}h"
                    )
            ...
            await safe_dm(
                after, dm_embed, cooldown_key=f"timeout_set:{guild.id}:{after.id}"
            )

        elif before_timeout and (
            not after_timeout or after_timeout <= discord.utils.utcnow()
        ):
            ...
            await safe_dm(
                after, dm_embed, cooldown_key=f"timeout_remove:{guild.id}:{after.id}"
            )
...
async def on_guild_channel_update(
    ...
    if getattr(before, "slowmode_delay", None) != getattr(
        ...
        diffs.append(
            (
                "Slowmode",
                f"`{before.slowmode_delay}s` → `{after.slowmode_delay}s`",
                True,
            )
        )
    ...
    if diffs:
        await bot.log_action(
            guild,
            f"{E.UPDATE} Channel aktualisiert",
    ...
    if before_ow != after_ow:
        perm_actor = (
            await _audit_actor(guild, discord.AuditLogAction.overwrite_update, after.id)
            or actor
        )
        changes: List[str] = []
        ...
        for target in all_targets:
            ...
            if b is None and a is not None:
                ...
                b_pair = (
                    b.pair()
                    if b
                    else (discord.Permissions.none(), discord.Permissions.none())
                )
                a_pair = (
                    a.pair()
                    if a
                ...
                if allow_diff or deny_diff:
                    ...
                    for perm_name, has in diff_perms:
                        if has:
                            bv = (
                                getattr(b_pair[0], perm_name),
                                getattr(b_pair[1], perm_name),
                            )
                            av = (
                                getattr(a_pair[0], perm_name),
                                getattr(a_pair[1], perm_name),
                            )
                            changed_perms.append(f"`{perm_name}`: {bv}→{av}")
                if changed_perms:
                    changes.append(
                        f"🔧 {target_label}: " + ", ".join(changed_perms[:6])
                    )
        if changes:
            desc = "\n".join(changes[:15])
            if len(changes) > 15:
            ...
            await bot.log_action(
                guild,
                f"{E.PERMS} Permissions geändert",
                f"**Channel:** {after.mention if hasattr(after, 'mention') else after.name}\n"
...
async def on_guild_role_update(before: discord.Role, after: discord.Role) -> None:
    ...
    if before.mentionable != after.mentionable:
        diffs.append(
            ("Erwähnbar", f"`{before.mentionable}` → `{after.mentionable}`", True)
        )
    if before.permissions != after.permissions:
        ...
        if added_p:
            diffs.append(
                (
                    f"{E.PLUS} Permissions hinzugefügt",
                    "`" + "`, `".join(sorted(added_p))[:900] + "`",
                    False,
                )
            )
        if removed_p:
            diffs.append(
                (
                    f"{E.MINUS} Permissions entfernt",
                    "`" + "`, `".join(sorted(removed_p))[:900] + "`",
                    False,
                )
            )
...
async def on_guild_join(guild: discord.Guild) -> None:
    ...
    log.info(
        f"Bot zu Guild beigetreten: {guild.name} ({guild.id}) – {guild.member_count} Member"
    )
    ACTIVITY.push(
        "guild_join",
    ...
    embed.set_footer(
        ...
        icon_url=FOOTER_ICON
        if FOOTER_ICON
        else (bot.user.display_avatar.url if bot.user.display_avatar else None),
    )
    ...
    try:
        ...
        log.info(
            f"on_guild_join: Welcome-Embed gesendet in #{target_channel.name} ({guild.name})"
        )
    except discord.Forbidden:
        log.warning(
            f"on_guild_join: Keine Rechte in #{target_channel.name} ({guild.name})"
        )
    except discord.HTTPException as ex:
...
async def prefix_ban(
    ...
    *,
    ...
    if not ok:
        await ctx.send(
            embed=create_embed(f"{E.FAIL} Aktion blockiert", why, COLOR_DANGER)
        )
        return
    try:
        ...
        await ctx.send(
            embed=create_embed(
                f"{E.FAIL} Forbidden", "Ich darf das nicht.", COLOR_DANGER
            )
        )
        return
    embed = create_embed(
...
async def prefix_mute(
    ...
    *,
    ...
    embed = create_embed(
        f"{E.MUTE} Gemutet",
        f"{member.mention} wurde für {duration}s gemutet.",
        COLOR_WARNING,
...
async def prefix_unmute(ctx: commands.Context, member: discord.Member) -> None:
    ...
    embed = create_embed(
        f"{E.UNMUTE} Entmutet", f"{member.mention} wurde entmutet.", COLOR_SUCCESS
    )
    await ctx.send(embed=embed)
    await bot.log_action(
        ctx.guild,
...
async def prefix_clear(ctx: commands.Context, amount: int = 10) -> None:
    try:
        ...
        return
    embed = create_embed(
        f"{E.DELETE} Nachrichten gelöscht",
...
async def prefix_slowmode(ctx: commands.Context, seconds: int = 0) -> None:
    try:
        await ctx.channel.edit(
            slowmode_delay=seconds, reason=f"Slowmode durch {ctx.author}"
        )
    except discord.Forbidden:
        await ctx.send(
            embed=create_embed(
    ...
    embed = create_embed(
        f"{E.SLOW} Slowmode",
        f"Slowmode auf **{seconds}s** gesetzt."
        if seconds > 0
        else "Slowmode deaktiviert.",
        COLOR_INFO,
    )
    await ctx.send(embed=embed)
    await bot.log_action(
...
async def prefix_lock(ctx: commands.Context) -> None:
    try:
        ...
        return
    embed = create_embed(
        f"{E.LOCK} Kanal gesperrt",
        f"{ctx.channel.mention} wurde gesperrt.",
        COLOR_DANGER,
    )
    await ctx.send(embed=embed)
    await bot.log_action(
...
async def prefix_unlock(ctx: commands.Context) -> None:
    try:
        ...
        return
    embed = create_embed(
        f"{E.UNLOCK} Kanal entsperrt",
        f"{ctx.channel.mention} wurde entsperrt.",
        COLOR_SUCCESS,
    )
    await ctx.send(embed=embed)
    await bot.log_action(
    ...
    if not warns:
        ...
        for i, w in enumerate(warns):
            ...
            fields.append(
                (
                    f"#{i} – {w['reason'][:80]}",
                    f"{ts_text} • Mod: <@{w.get('mod_id', '?')}>",
                    False,
                )
            )
        embed = create_embed(
            f"{E.CHANNEL} Verwarnungen – {member.display_name}",
            f"**{len(warns)}** Verwarnungen insgesamt.",
            COLOR_WARNING,
            fields,
...
async def prefix_clearwarnings(ctx: commands.Context, member: discord.Member) -> None:
    ...
    embed = create_embed(
        f"{E.DELETE} Verwarnungen gelöscht",
...
async def prefix_unban(ctx: commands.Context, user_id: int) -> None:
    try:
        ...
        await ctx.send(
            embed=create_embed(
                f"{E.FAIL} Nicht gefunden", "Nutzer/Ban nicht gefunden.", COLOR_DANGER
            )
        )
        return
    except discord.Forbidden:
        ...
        return
    embed = create_embed(
        f"{E.OK} Entbannt", f"**{user}** wurde entbannt.", COLOR_SUCCESS
    )
    await ctx.send(embed=embed)
    await bot.log_action(
        ctx.guild,
...
@has_mod_perms()
async def prefix_nick(
    ctx: commands.Context, member: discord.Member, *, nickname: str = None
) -> None:
    try:
...
async def prefix_role(
    ctx: commands.Context, action: str, member: discord.Member, role: discord.Role
) -> None:
    try:
        if action.lower() == "add":
            await member.add_roles(role, reason=f"Role-Add durch {ctx.author}")
            embed = create_embed(
                f"{E.ROLE_ADD} Rolle hinzugefügt",
            ...
            embed = create_embed(
                f"{E.ROLE_DEL} Rolle entfernt",
                f"{role.mention} wurde {member.mention} entfernt.",
                COLOR_WARNING,
            )
        else:
            embed = create_embed(
                f"{E.FAIL} Fehler", "Nutze `add` oder `remove`.", COLOR_DANGER
            )
    except discord.Forbidden:
...
async def cmd_banappeal(
    ctx: commands.Context, user_id: int, *, ban_reason: str = "Kein Grund angegeben"
) -> None:
    cfg = bot.db.get_config(ctx.guild.id)
    if not cfg.get("appeal_log_channel"):
        await ctx.send(
            embed=create_embed(
                f"{E.ALERT}️ Kein Appeal-Kanal gesetzt",
    ...
    try:
        ...
        await ctx.send(
            embed=create_embed(
                f"{E.FAIL} Nutzer nicht gefunden",
    ...
    try:
        ...
        embed_confirm = create_embed(
            f"{E.OK} Appeal-DM gesendet",
...
# ═══════════════════════════════════════════════════════════════
@bot.tree.command(
    name="setup_verify", description="Richtet das Verifizierungssystem ein"
)
@app_commands.describe(
    channel="Kanal für die Nachricht",
    mode="Modus (One-Click oder Captcha)",
    difficulty="Captcha-Stärke",
)
@app_commands.choices(
    mode=[
        app_commands.Choice(name="One-Click", value="one_click"),
        app_commands.Choice(name="Captcha", value="captcha"),
    ],
    difficulty=[
        app_commands.Choice(name="Leicht", value="easy"),
...
async def setup_verify(
    ...
    embed = create_embed(
        f"{E.SHIELD} Verifizierung erforderlich",
    ...
    await bot.db.set_config(interaction.guild.id, cfg)
    await interaction.response.send_message(
        f"{E.OK} Verifizierung in {channel.mention} eingerichtet!", ephemeral=True
    )


@bot.tree.command(
    name="verify_roles", description="Stellt die Rollen für die Verifizierung ein"
)
@app_commands.describe(
    add_role="Rolle die gegeben wird", remove_role="Rolle die entfernt wird"
)
@app_commands.default_permissions(administrator=True)
async def verify_roles(
    ...
    await bot.db.set_config(interaction.guild.id, cfg)
    await interaction.response.send_message(
        f"{E.OK} Verifizierungs-Rollen aktualisiert!", ephemeral=True
    )


@bot.tree.command(name="setup_tickets", description="Richtet das Ticket-System ein")
@app_commands.describe(
    category="Kategorie für Tickets", log_channel="Kanal für Ticket-Logs"
)
@app_commands.default_permissions(administrator=True)
async def setup_tickets(
    ...
    cfg = bot.db.get_config(interaction.guild.id)
    cfg["ticket_system"].update(
        {"enabled": True, "category_id": category.id, "log_channel_id": log_channel.id}
    )
    embed = create_embed(
        f"{E.TICKETBOX} Support Tickets",
    ...
    await bot.db.set_config(interaction.guild.id, cfg)
    await interaction.response.send_message(
        f"{E.OK} Ticket-System eingerichtet!", ephemeral=True
    )
...
async def slash_ban(
    interaction: discord.Interaction, member: discord.Member, reason: str = "Kein Grund"
) -> None:
    ...
    pre_msgs = await bot.db.aget_user_messages(
        interaction.guild.id, member.id, hours=48, limit=500
    )
    try:
        ...
        serializable = [
            {
                ...
                "deleted": bool(m.get("deleted")),
                "edits": m.get("edits", []),
                "timestamp": (m.get("timestamp") or discord.utils.utcnow()).isoformat()
                + "Z",
            }
            for m in pre_msgs
        ]
        await bot.db.aattach_messages_to_case(
            interaction.guild.id, case_id, serializable
        )
    ACTIVITY.push(
        "case",
        f"Case #{case_id} (ban) – {member} via /ban",
        guild_id=interaction.guild.id,
...
@app_commands.describe(
    member="Der zu bannende Nutzer", duration="Dauer (z.B. 30m, 2h, 7d)", reason="Grund"
)
@app_commands.default_permissions(ban_members=True)
async def slash_tempban(
    ...
    if not seconds or seconds < 60:
        await interaction.response.send_message(
            embed=create_embed(
                f"{E.FAIL} Ungültige Dauer",
                "Format: `30m`, `2h`, `7d` (mind. 60s)",
                COLOR_DANGER,
            ),
            ephemeral=True,
        )
    ...
    try:
        ...
        return
    await bot.db.aadd_tempaction(
        "tempban", interaction.guild.id, member.id, end, reason, interaction.user.id
    )
    ...
    await bot.log_action(
        interaction.guild,
        f"{E.BAN} Tempban – Case #{case_id}",
        f"{member.mention} – Tempban {duration}",
        COLOR_DANGER,
        [
            ("Grund", reason, False),
            ("Endet", f"<t:{int(end.timestamp())}:R>", True),
...
@bot.tree.command(
    name="tempmute", description="Persistenter Timeout (überlebt Neustart)"
)
@app_commands.describe(member="Nutzer", duration="Dauer (z.B. 30m, 2h)", reason="Grund")
@app_commands.default_permissions(moderate_members=True)
async def slash_tempmute(
    interaction: discord.Interaction,
    member: discord.Member,
    duration: str,
    reason: str = "Kein Grund",
    ...
    try:
        await member.timeout(
            end, reason=f"Tempmute {duration} – {interaction.user}: {reason}"
        )
    except discord.Forbidden:
        await interaction.response.send_message(
            embed=create_embed(
        ...
        return
    await bot.db.aadd_mute(
        interaction.guild.id, member.id, reason, interaction.user.id, seconds
    )
    await bot.db.aadd_tempaction(
        "tempmute", interaction.guild.id, member.id, end, reason, interaction.user.id
    )
    case_id = await bot.create_mod_case(
        interaction.guild,
        member,
    ...
    embed = create_embed(
        f"{E.MUTE} Tempmute – Case #{case_id}",
...
async def slash_warn(
    ...
    count = await bot.db.aadd_warning(
    ...
    await bot._check_warn_thresholds(member, count)
    embed = create_embed(
        f"{E.WARN} Verwarnt – Case #{case_id}",
        f"{member.mention} wurde verwarnt.",
        COLOR_WARNING,
        [
            ("Grund", reason, False),
...
async def slash_mute(
    ...
    try:
        ...
        return
    await bot.db.aadd_mute(
        interaction.guild.id, member.id, reason, interaction.user.id, duration
    )
    case_id = await bot.create_mod_case(
        interaction.guild,
        member,
    ...
    embed = create_embed(
        f"{E.MUTE} Gemutet – Case #{case_id}",
        f"{member.mention} wurde für {duration}s gemutet.",
        COLOR_WARNING,
        [
            ("Grund", reason, False),
            ("Dauer", f"{duration}s", True),
            ("Case-ID", f"`{case_id}`", True),
        ],
    )
    await interaction.response.send_message(embed=embed)
...
async def slash_slowmode(interaction: discord.Interaction, seconds: int = 0) -> None:
    try:
        await interaction.channel.edit(
            slowmode_delay=seconds, reason=f"Slowmode durch {interaction.user}"
        )
    except discord.Forbidden:
        await interaction.response.send_message(
            embed=create_embed(
...
async def slash_unban(interaction: discord.Interaction, user_id: str) -> None:
    try:
        ...
        await interaction.response.send_message(
            embed=create_embed(
                f"{E.FAIL} Ungültige ID", "Bitte eine Zahl angeben.", COLOR_DANGER
            ),
            ephemeral=True,
        )
        return
...
@bot.tree.command(
    name="clearwarnings", description="Löscht ALLE Verwarnungen eines Nutzers"
)
@app_commands.describe(member="Nutzer")
@app_commands.default_permissions(manage_messages=True)
...
@bot.tree.command(
    name="clearwarning",
    description="Löscht eine einzelne Verwarnung (Index aus /warnings)",
)
@app_commands.describe(member="Nutzer", index="Index aus /warnings (#0, #1, ...)")
@app_commands.default_permissions(manage_messages=True)
async def slash_clearwarning(
    interaction: discord.Interaction, member: discord.Member, index: int
) -> None:
    success = await bot.db.aremove_warning(interaction.guild.id, member.id, index)
    if success:
        embed = create_embed(
            f"{E.OK} Verwarnung entfernt",
            f"Verwarnung #{index} von {member.mention} entfernt.",
            COLOR_SUCCESS,
        )
        await bot.log_action(
            interaction.guild,
            f"{E.OK} Clear-Warning",
        ...
        embed = create_embed(
            f"{E.FAIL} Nicht gefunden", f"Index {index} existiert nicht.", COLOR_DANGER
        )
    await interaction.response.send_message(embed=embed)
...
async def slash_nick(
    interaction: discord.Interaction, member: discord.Member, nickname: str = None
) -> None:
    try:
...
security_group = app_commands.Group(
    name="security", description="Security-Einstellungen verwalten"
)


@security_group.command(
    name="view", description="Zeigt die aktuelle Security-Konfiguration"
)
@app_commands.default_permissions(administrator=True)
...
async def security_toggle(interaction: discord.Interaction, module: str) -> None:
    ...
    if module not in cfg or not isinstance(cfg[module], dict):
        await interaction.response.send_message(
            embed=create_embed(
                f"{E.FAIL} Fehler", f"Modul `{module}` nicht gefunden.", COLOR_DANGER
            ),
            ephemeral=True,
        )
        return
...
@app_commands.default_permissions(administrator=True)
async def security_set(
    interaction: discord.Interaction, module: str, setting: str, value: str
) -> None:
    parsed: Any = value
    ...
    embed = create_embed(
        f"{E.OK} Einstellung gespeichert",
        f"`{module}.{setting}` = `{parsed}`",
        COLOR_SUCCESS,
    )
    await interaction.response.send_message(embed=embed)
...
@app_commands.default_permissions(administrator=True)
async def security_punishment(
    interaction: discord.Interaction, module: str, punishment_type: str
) -> None:
    if punishment_type not in VALID_PUNISHMENTS:
        await interaction.response.send_message(
            embed=create_embed(
                f"{E.FAIL} Ungültige Strafe",
                f"Erlaubte Werte: {', '.join(VALID_PUNISHMENTS)}",
                COLOR_DANGER,
            ),
            ephemeral=True,
        )
        return
    await bot.db.update_module(
        interaction.guild.id, module, "punishment", punishment_type
    )
    embed = create_embed(
        f"{E.OK} Strafe gesetzt",
        f"Strafe für **{module}** auf **{punishment_type}** gesetzt.",
        COLOR_SUCCESS,
    )
    await interaction.response.send_message(embed=embed)
...
@security_group.command(
    name="reset", description="Setzt ein Modul auf Standardwerte zurück"
)
...
async def security_reset(interaction: discord.Interaction, module: str) -> None:
    ...
    await bot.db.set_config(interaction.guild.id, cfg)
    embed = create_embed(
        f"{E.DUP} Modul zurückgesetzt",
        f"**{module}** wurde auf Standardwerte zurückgesetzt.",
        COLOR_INFO,
    )
    await interaction.response.send_message(embed=embed)
...
whitelist_group = app_commands.Group(
    name="whitelist", description="Whitelist verwalten"
)
...
@whitelist_group.command(
    name="add", description="Fügt einen Eintrag zur Whitelist hinzu"
)
@app_commands.describe(category="Kategorie wählen", entry_id="ID des Eintrags")
@app_commands.choices(category=WL_CATEGORIES)
@app_commands.default_permissions(administrator=True)
async def whitelist_add(
    interaction: discord.Interaction, category: app_commands.Choice[str], entry_id: str
) -> None:
...
@whitelist_group.command(
    name="bulk_add", description="Fügt viele IDs gleichzeitig hinzu"
)
@app_commands.describe(
    category="Kategorie wählen", ids="IDs getrennt durch Komma oder Leerzeichen"
)
@app_commands.choices(category=WL_CATEGORIES)
@app_commands.default_permissions(administrator=True)
async def whitelist_bulk_add(
    interaction: discord.Interaction, category: app_commands.Choice[str], ids: str
) -> None:
...
@whitelist_group.command(
    name="remove", description="Entfernt einen Eintrag aus der Whitelist"
)
...
async def whitelist_remove(
    ...
    embed = create_embed(
        f"{E.OK} Entfernt",
        f"ID `{entry_id}` aus **{category.name}** entfernt.",
        COLOR_SUCCESS,
    )
    await interaction.response.send_message(embed=embed)
...
@whitelist_group.command(
    name="add_user", description="Fügt einen Nutzer bequem zur Whitelist hinzu"
)
...
@whitelist_group.command(
    name="add_role", description="Fügt eine Rolle bequem zur Whitelist hinzu"
)
...
async def whitelist_add_role(
    ...
    embed = create_embed(
        f"{E.OK} Rolle hinzugefügt",
        f"{role.mention} wurde zur Whitelist hinzugefügt.",
        COLOR_SUCCESS,
    )
    await interaction.response.send_message(embed=embed)
...
@whitelist_group.command(
    name="add_channel", description="Fügt einen Kanal bequem zur Whitelist hinzu"
)
...
async def whitelist_add_channel(
    ...
    embed = create_embed(
        f"{E.OK} Kanal hinzugefügt",
        f"{channel.mention} wurde zur Whitelist hinzugefügt.",
        COLOR_SUCCESS,
    )
    await interaction.response.send_message(embed=embed)
...
@whitelist_group.command(
    name="remove_role", description="Entfernt eine Rolle bequem aus der Whitelist"
)
@app_commands.describe(role="Rolle")
@app_commands.default_permissions(administrator=True)
async def whitelist_remove_role(
    ...
    embed = create_embed(
        f"{E.OK} Rolle entfernt",
        f"{role.mention} wurde aus der Whitelist entfernt.",
        COLOR_SUCCESS,
    )
    await interaction.response.send_message(embed=embed)
...
@whitelist_group.command(
    name="clear", description="Leert eine Whitelist-Kategorie komplett"
)
...
async def whitelist_clear(
    ...
    embed = create_embed(
        f"{E.OK} Geleert",
        f"Kategorie **{category.name}** wurde komplett geleert.",
        COLOR_SUCCESS,
    )
    await interaction.response.send_message(embed=embed)
...
async def whitelist_list(interaction: discord.Interaction) -> None:
    ...
    fields = [
        (
            f"{E.USERS} Nutzer",
            "\n".join(f"<@{uid}>" for uid in wl.get("users", [])) or "Keine",
            False,
        ),
        (
            f"{E.ROLES} Rollen",
            "\n".join(f"<@&{rid}>" for rid in wl.get("roles", [])) or "Keine",
            False,
        ),
        (
            f"{E.CHANNELS} Kanäle",
            "\n".join(f"<#{cid}>" for cid in wl.get("channels", [])) or "Keine",
            False,
        ),
        (
            f"{E.SPAM} Bypass Anti-Spam",
            "\n".join(f"<@{uid}>" for uid in wl.get("bypass_antispam", [])) or "Keine",
            False,
        ),
        (
            f"{E.NUKE} Bypass Anti-Nuke",
            "\n".join(f"<@{uid}>" for uid in wl.get("bypass_antinuke", [])) or "Keine",
            False,
        ),
    ]
    embed = create_embed(
        f"{E.CHANNEL} Whitelist",
        f"Whitelist für **{interaction.guild.name}**",
        COLOR_INFO,
        fields,
    )
    await interaction.response.send_message(embed=embed)
...
async def automod_badword_add(interaction: discord.Interaction, word: str) -> None:
    ...
    if word.lower() not in bad_words:
        ...
        await bot.db.update_module(
            interaction.guild.id, "automod", "bad_words", bad_words
        )
    embed = create_embed(
        f"{E.OK} Wort hinzugefügt",
        f"`{word}` zur Wortliste hinzugefügt.",
        COLOR_SUCCESS,
    )
    await interaction.response.send_message(embed=embed)
...
@automod_group.command(
    name="badword_remove", description="Entfernt ein verbotenes Wort"
)
...
async def automod_badword_remove(interaction: discord.Interaction, word: str) -> None:
    ...
    embed = create_embed(
        f"{E.OK} Wort entfernt", f"`{word}` aus der Wortliste entfernt.", COLOR_SUCCESS
    )
    await interaction.response.send_message(embed=embed)
...
async def automod_badword_list(interaction: discord.Interaction) -> None:
    ...
    if not bad_words:
        embed = create_embed(
            f"{E.CHANNEL} Verbotene Wörter",
            "Keine verbotenen Wörter konfiguriert.",
            COLOR_INFO,
        )
    else:
...
@automod_group.command(
    name="regex_add", description="Fügt eine Custom Regex-Regel hinzu"
)
...
@automod_group.command(
    name="regex_remove", description="Entfernt eine Regex-Regel per Index"
)
...
async def automod_regex_remove(interaction: discord.Interaction, index: int) -> None:
    ...
    if not (0 <= index < len(rules)):
        await interaction.response.send_message(
            embed=create_embed(
                f"{E.FAIL} Ungültig", f"Index {index} existiert nicht.", COLOR_DANGER
            ),
            ephemeral=True,
        )
        return
    ...
    await bot.db.update_module(interaction.guild.id, "automod", "regex_rules", rules)
    embed = create_embed(
        f"{E.OK} Regex entfernt", f"Entfernt: `{removed[:200]}`", COLOR_SUCCESS
    )
    await interaction.response.send_message(embed=embed)
...
async def automod_regex_list(interaction: discord.Interaction) -> None:
    ...
    if not rules:
        embed = create_embed(
            f"{E.CHANNEL} Regex-Regeln", "Keine Custom-Regeln konfiguriert.", COLOR_INFO
        )
    else:
        fields = [(f"#{i}", f"`{p[:200]}`", False) for i, p in enumerate(rules)]
        embed = create_embed(
            f"{E.CHANNEL} Regex-Regeln",
            f"**{len(rules)}** Regeln aktiv.",
            COLOR_WARNING,
            fields,
        )
    await interaction.response.send_message(embed=embed, ephemeral=True)
...
@automod_group.command(
    name="domain_allow", description="Erlaubt eine Domain im Link-Filter"
)
...
async def automod_domain_allow(interaction: discord.Interaction, domain: str) -> None:
    ...
    if domain.lower() not in allowed:
        ...
        await bot.db.update_module(
            interaction.guild.id, "automod", "allowed_domains", allowed
        )
    embed = create_embed(
        f"{E.OK} Domain erlaubt",
        f"`{domain}` zur Whitelist hinzugefügt.",
        COLOR_SUCCESS,
    )
    await interaction.response.send_message(embed=embed)
...
async def slash_case(interaction: discord.Interaction, case_id: int) -> None:
    ...
    user = interaction.guild.get_member(case["user_id"]) or await bot.fetch_user(
        case["user_id"]
    )
    mod = interaction.guild.get_member(case["mod_id"]) or await bot.fetch_user(
        case["mod_id"]
    )
...
async def slash_reason(
    ...
    await interaction.response.send_message(
        embed=create_embed(
            f"{E.OK} Case #{case_id} aktualisiert",
            f"Neuer Grund:\n```\n{grund[:500]}\n```",
            COLOR_SUCCESS,
        ),
        ephemeral=True,
...
@bot.tree.command(
    name="addproof", description="Fügt einem Case einen Beweis (URL/Screenshot) hinzu"
)
@app_commands.describe(
    case_id="Die Case-ID",
    ...
    await interaction.response.send_message(
        embed=create_embed(
            ...
            (
                ...
                if enabled
                else "Das Archiv wurde deaktiviert. Bestehende Einträge laufen via "
...
async def slash_audit_perms(interaction: discord.Interaction) -> None:
    ...
    danger_perms: List[str] = audit_cfg.get(
        "danger_perms",
        [
            "administrator",
            "manage_guild",
    ...
    higher_roles = [
        r for r in guild.roles if r.position > bot_top.position and not r.is_default()
    ]
    if higher_roles:
        findings.append(
            (
...
async def slash_logs(
    ...
    await interaction.response.defer(
        ephemeral=True
    )  # ← NEU: sofort Bescheid geben, dass es dauert
    cfg = bot.db.get_config(interaction.guild.id)
    cfg["log_channel"] = channel.id
    await bot.db.set_config(interaction.guild.id, cfg)
    embed = create_embed(
        f"{E.CHANNEL} Standard-Log-Kanal gesetzt",
...
async def slash_logset(
    ...
    await bot.db.set_config(interaction.guild.id, cfg)
    embed = create_embed(
        f"{E.OK} Modul-Log-Kanal gesetzt",
...
@bot.tree.command(
    name="logreset",
    description="Setzt den Log-Kanal für ein Modul zurück (auf Standard)",
)
@app_commands.describe(module="Modul wählen")
@app_commands.choices(module=_module_choices())
@app_commands.default_permissions(administrator=True)
async def slash_logreset(
    interaction: discord.Interaction, module: app_commands.Choice[str]
) -> None:
    cfg = bot.db.get_config(interaction.guild.id)
    log_channels = cfg.get("log_channels", {}) or {}
    ...
    await bot.db.set_config(interaction.guild.id, cfg)
    embed = create_embed(
        f"{E.OK} Modul-Log zurückgesetzt",
...
async def slash_logview(interaction: discord.Interaction) -> None:
    ...
    fields = [
        (
            "Standard / Fallback",
            f"<#{default_id}>" if default_id else "*Nicht gesetzt*",
            False,
        )
    ]
...
# ═══════════════════════════════════════════════════════════════
@bot.tree.command(
    name="security_level", description="Setzt die Sicherheitsstufe des Servers"
)
...
@app_commands.choices(
    level=[
        ...
        app_commands.Choice(
            name="Level 2: Sturmflut (Auto-Kick neue Accounts)", value=2
        ),
        app_commands.Choice(
            name="Level 3: BLACKOUT (Server eingefroren, Joins = Ban)", value=3
        ),
    ]
)
@app_commands.default_permissions(administrator=True)
async def slash_security_level(
    ...
    embed = create_embed(
        f"{E.SHIELD} Sicherheitsstufe geändert",
...
async def slash_panic(interaction: discord.Interaction) -> None:
    ...
    bot.tracker.lockdown_active[interaction.guild.id] = True
    embed = create_embed(
        f"{E.NUKE} PANIC – BLACKOUT AKTIVIERT",
        "Sicherheitsstufe 3 wurde sofort aktiviert. Alle Kanäle gesperrt!",
        COLOR_DANGER,
        [("Ausgeführt von", interaction.user.mention, True)],
    )
...
async def slash_unlockdown(interaction: discord.Interaction) -> None:
    ...
    embed = create_embed(
        f"{E.LOCK} Lockdown aufgehoben", "Alle Kanäle wurden entsperrt.", COLOR_SUCCESS
    )
    await interaction.followup.send(embed=embed)
...
@bot.tree.command(
    name="massunban",
    description="Entbannt alle gebannten Nutzer (Rate-Limit-geschützt)",
)
@app_commands.default_permissions(administrator=True)
async def slash_massunban(interaction: discord.Interaction) -> None:
    ...
    for ban_entry in bans:
        try:
            ...
            pass
    embed = create_embed(
        f"{E.OK} Mass-Unban abgeschlossen",
        f"**{count}** von {len(bans)} Nutzern wurden entbannt.",
        COLOR_SUCCESS,
    )
    await interaction.followup.send(embed=embed)
    await bot.log_action(
...
@bot.tree.command(
    name="massrole_remove",
    description="Entfernt eine Rolle von allen Mitgliedern (Rate-Limit-geschützt)",
)
@app_commands.describe(role="Die zu entfernende Rolle")
@app_commands.default_permissions(administrator=True)
async def slash_massrole_remove(
    interaction: discord.Interaction, role: discord.Role
) -> None:
    await interaction.response.defer()
    count = 0
    ...
    for member in targets:
        try:
            await rate_limited(
                lambda m=member: m.remove_roles(
                    role, reason=f"Mass-Role-Remove durch {interaction.user}"
                )
            )
            count += 1
        except discord.Forbidden:
...
@bot.tree.command(
    name="forcereset", description="Setzt die gesamte Server-Konfiguration zurück"
)
@app_commands.default_permissions(administrator=True)
...
@bot.tree.command(
    name="warnsetup", description="Konfiguriert Warn-Schwellen interaktiv"
)
@app_commands.default_permissions(administrator=True)
...
async def on_command_error(ctx: commands.Context, error: Exception) -> None:
    if isinstance(error, commands.MissingPermissions):
        ...
        embed = create_embed(
            f"{E.FAIL} Ungültiges Argument",
            f"Ungültiges Argument: `{error}`",
            COLOR_DANGER,
        )
        await ctx.send(embed=embed)
    elif isinstance(error, commands.CheckFailure):
        embed = create_embed(
            f"{E.FAIL} Zugriff verweigert",
            "Du hast keine Berechtigung für diesen Command.",
            COLOR_DANGER,
        )
        await ctx.send(embed=embed)
...
async def on_app_command_error(
    ...
    if user_msg is not None:
        log.warning(
            f"Slash-Cmd '{cmd_name}' von {interaction.user}: {type(error).__name__}: {error}"
        )
        return
...
async def _collect_backup_data(guild: discord.Guild) -> dict:
    ...
    data = {
        ...
        "public_updates_channel_id": guild.public_updates_channel.id
    ...
    # Emojis
    data["emojis"] = [
        {"emoji_id": e.id, "name": e.name, "url": str(e.url), "animated": e.animated}
...
def _deserialize_overwrites(
    guild: discord.Guild, overwrites_data: list, role_map: dict
) -> dict:
    result = {}
...
async def _restore_from_backup(
    guild: discord.Guild, backup: dict, interaction: discord.Interaction
) -> dict:
    report = {
        "roles_created": 0,
        "roles_updated": 0,
        ...
        try:
            ...
            if existing and isinstance(existing, discord.CategoryChannel):
                try:
                    await existing.edit(
                        name=cat_data["name"],
                        overwrites=_deserialize_overwrites(
                            guild, cat_data.get("overwrites", []), role_map
                        ),
    ...
    for ch_data in backup.get("channels", []):
        try:
            ...
            overwrites = _deserialize_overwrites(
                guild, ch_data.get("overwrites", []), role_map
            )
            ...
            if existing_by_name:
                try:
                    ...
                    if isinstance(
                        existing_by_name, discord.TextChannel
                    ) and ch_data.get("topic"):
                        kwargs["topic"] = ch_data["topic"]
                    if category:
                        kwargs["category"] = category
    ...
    try:
        await guild.edit(
            verification_level=discord.VerificationLevel(
                backup.get("verification_level", 0)
            ),
            explicit_content_filter=discord.ExplicitContentFilter(
                backup.get("explicit_content_filter", 0)
            ),
            default_notifications=discord.NotificationLevel(
                backup.get("default_notifications", 0)
            ),
            reason="Backup-Restore",
        )
    except (discord.Forbidden, discord.HTTPException) as e:
...
async def _backup_db_save(
    backup_data: dict, created_by: int, label: str = None, password_hash: str = None
) -> str:
    backup_id = _hashlib.sha256(
        f"{backup_data['guild_id']}_{time.time()}_{random.randint(0, 999999)}".encode()
    ).hexdigest()[:16]
    doc = {
        "backup_id": backup_id,
        ...
        "label": label
        or f"Backup vom {discord.utils.utcnow().strftime('%d.%m.%Y %H:%M')} UTC",
...
async def _backup_db_delete(guild_id: int, backup_id: str) -> bool:
    try:
        ...
        result = await collection.delete_one(
            {"guild_id": guild_id, "backup_id": backup_id}
        )
...
class BackupPasswordModal(discord.ui.Modal, title="🔐 Backup-Passwort eingeben"):
    password = discord.ui.TextInput(
        label="Passwort",
        placeholder="Gib das Backup-Passwort ein...",
    ...
    async def on_submit(self, interaction: discord.Interaction) -> None:
        ...
        if not doc or not doc.get("data"):
            embed = create_embed(
                f"{E.FAIL} Fehler", "Backup nicht gefunden.", COLOR_DANGER
            )
            await interaction.followup.send(embed=embed)
            return
        ...
        fields = [
            (
                "Rollen erstellt/aktualisiert/übersprungen",
                f"{E.OK} {report['roles_created']} / 🔄 {report['roles_updated']} / ⏭️ {report['roles_skipped']}",
                False,
            ),
            (
                "Kategorien",
                f"{E.OK} {report['categories_created']} / 🔄 {report['categories_updated']}",
                False,
            ),
            (
                "Kanäle",
                f"{E.OK} {report['channels_created']} / 🔄 {report['channels_updated']}",
                False,
            ),
        ]
        if report["errors"]:
        ...
        result_embed = create_embed(
            f"{E.OK} Backup-Restore abgeschlossen",
            f"Backup **{self.backup_label}** wurde wiederhergestellt.",
            result_color,
            fields,
        )
        ...
        await bot.log_action(
            self.guild,
            f"{E.OK} Backup-Restore",
            f"Backup **{self.backup_label}** von {self.user.mention} wiederhergestellt.",
            result_color,
            fields,
            user=self.user,
            module="backup",
...
class BackupRestoreConfirmView(discord.ui.View):
    ...
    async def confirm_restore(
        ...
        if interaction.user.id != self.user.id:
            await interaction.response.send_message(
                f"{E.FAIL} Nur der Initiator kann bestätigen.", ephemeral=True
            )
            return
        await interaction.response.defer()
        if self.has_password:
            embed = create_embed(
                ...
                "Dieses Backup ist passwortgeschützt. Bitte gib das Passwort ein.",
                COLOR_WARNING,
            )
            await interaction.followup.send(
                embed=embed,
                view=BackupPasswordPromptView(
                    self.backup_id, self.backup_label, self.guild, self.user
                ),
            )
            return
        doc = await _backup_db_get(self.guild.id, self.backup_id)
        if not doc or not doc.get("data"):
            ...
            await interaction.followup.send(embed=embed)
            return
        embed_progress = create_embed(
            "⏳ Backup-Restore läuft...",
            f"Backup **{self.backup_label}** wird wiederhergestellt.",
            COLOR_WARNING,
        )
        ...
        await interaction.channel.send(
            embed=create_embed(
                f"{E.OK} Backup-Restore abgeschlossen",
                f"Backup **{self.backup_label}** wiederhergestellt.",
                rc,
                fields,
            )
        )
    ...
    @discord.ui.button(
        label="Abbrechen", style=discord.ButtonStyle.secondary, emoji=E.FAIL
    )
...
class BackupPasswordPromptView(discord.ui.View):
    ...
    @discord.ui.button(
        label="Passwort eingeben", style=discord.ButtonStyle.primary, emoji="🔑"
    )
    async def enter_pw(
        self, interaction: discord.Interaction, button: discord.ui.Button
    ) -> None:
        if interaction.user.id != self.user.id:
            await interaction.response.send_message(
                f"{E.FAIL} Nur der Initiator.", ephemeral=True
            )
            return
        await interaction.response.send_modal(
            BackupPasswordModal(
...
class BackupDeleteConfirmView(discord.ui.View):
    def __init__(
        self, backup_id: str, backup_label: str, guild_id: int, user: discord.Member
    ) -> None:
        super().__init__(timeout=60)
        self.backup_id = backup_id
        self.backup_label = backup_label
    ...
    async def confirm_delete(
        ...
        if success:
            await interaction.response.edit_message(
                embed=create_embed(
                    f"{E.DELETE} Backup gelöscht",
                    f"Backup **{self.backup_label}** gelöscht.",
                    COLOR_SUCCESS,
                ),
                view=None,
            )
            ...
            await interaction.response.edit_message(
                embed=create_embed(
                    f"{E.FAIL} Fehler", "Konnte nicht löschen.", COLOR_DANGER
                ),
                view=None,
            )
        await bot.log_action(
            interaction.guild,
            f"{E.DELETE} Backup gelöscht",
            f"Backup **{self.backup_label}** von {self.user.mention} gelöscht.",
            COLOR_WARNING,
            user=self.user,
            module="backup",
        )
    ...
    async def cancel_delete(
        ...
        await interaction.response.edit_message(
            embed=create_embed(
                "🚫 Abgebrochen", "Backup wurde NICHT gelöscht.", COLOR_WARNING
            ),
            view=None,
        )
...
class BackupSetPasswordModal(discord.ui.Modal, title="🔐 Backup mit Passwort sichern"):
    ...
    label = discord.ui.TextInput(
        ...
        placeholder="z.B. Vor Server-Reset",
        style=discord.TextStyle.short,
        required=False,
        max_length=100,
    )
    ...
    async def on_submit(self, interaction: discord.Interaction) -> None:
        ...
        backup_id = await _backup_db_save(
            backup_data,
            interaction.user.id,
        ...
        if not backup_id:
            ...
            return
        backup_data.get("stats", {})
        fields = [
            ("Backup-ID", f"`{backup_id}`", True),
            ("🔐 Passwortgeschützt", "{E.OK} Ja", True),
            ("Label", self.label.value or "Automatisch", True),
            ("Rollen", str(len(backup_data.get("roles", []))), True),
            ("Kanäle", str(len(backup_data.get("channels", []))), True),
            ("Kategorien", str(len(backup_data.get("categories", []))), True),
        ]
        await interaction.followup.send(
            embed=create_embed(
                f"{E.OK} Passwortgeschütztes Backup erstellt",
                f"Backup **{backup_id}** wurde mit Passwort gesichert.",
                COLOR_SUCCESS,
                fields,
            )
        )
        await bot.log_action(
            guild,
            f"{E.OK} Passwort-Backup erstellt",
            f"Backup `{backup_id}` (passwortgeschützt) von {interaction.user.mention}.",
            COLOR_SUCCESS,
            user=interaction.user,
            module="backup",
        )
    ...
    def __init__(self) -> None:
        options = [
            discord.SelectOption(
                ...
                value="create",
                description="Neues Backup des Servers erstellen",
                emoji="📦",
            ),
            discord.SelectOption(
                label="🔐 Passwort-Backup erstellen",
                value="create_pw",
                description="Backup mit Passwort sichern",
                emoji="🔐",
            ),
            discord.SelectOption(
                label="📋 Backups anzeigen",
                value="list",
                description="Alle Backups auflisten",
                emoji="📋",
            ),
            discord.SelectOption(
                label="🔍 Backup-Details",
                value="info",
                description="Details zu einem Backup anzeigen",
                emoji="🔍",
            ),
            discord.SelectOption(
                label=f"{E.REFRESH}️ Backup wiederherstellen",
                value="restore",
                description="Backup auf diesem Server restoren",
                emoji="{E.REFRESH}️",
            ),
            discord.SelectOption(
                label="🌐 Cross-Server Restore",
                value="restore_cross",
                description="Backup von einem anderen Server restoren",
                emoji="🌐",
            ),
            discord.SelectOption(
                label="🗑️ Backup löschen",
                value="delete",
                description="Ein Backup löschen",
                emoji="🗑️",
            ),
            discord.SelectOption(
                label="💣 Alle Backups löschen",
                value="purge",
                description="ALLE Backups dieses Servers löschen",
                emoji="💣",
            ),
            discord.SelectOption(
                label=f"{E.GEAR}️ Auto-Backup konfigurieren",
                value="autosetup",
                description="Automatische Backups einstellen",
                emoji="{E.GEAR}️",
            ),
        ]
        super().__init__(
            placeholder="🔧 Backup-Aktion wählen...",
            options=options,
            min_values=1,
            max_values=1,
        )
    ...
    async def callback(self, interaction: discord.Interaction) -> None:
        ...
        if val == "create":
            ...
            if not backup_id:
                await interaction.response.send_message(
                    embed=create_embed(
                        f"{E.FAIL} Fehler", "Backup fehlgeschlagen.", COLOR_DANGER
                    ),
                    ephemeral=True,
                )
                return
            ...
            fields = [
                ...
                ("Kanäle", str(len(backup_data.get("channels", []))), True),
                ("Kategorien", str(len(backup_data.get("categories", []))), True),
                ("Mitglieder", str(stats.get("total_members", "?")), True),
                ("Emojis", str(len(backup_data.get("emojis", []))), True),
            ]
            await interaction.response.send_message(
                embed=create_embed(
                    f"{E.OK} Backup erstellt",
                    f"Backup **{backup_id}** gespeichert.",
                    COLOR_SUCCESS,
                    fields,
                ),
                ephemeral=True,
            ...
            await bot.log_action(
                interaction.guild,
                f"{E.OK} Backup erstellt",
                f"Backup `{backup_id}` von {interaction.user.mention}.",
                COLOR_SUCCESS,
                user=interaction.user,
                module="backup",
            )
            ...
            if not backups:
                await interaction.followup.send(
                    embed=create_embed(
                        f"{E.CHANNEL} Backups", "Keine Backups vorhanden.", COLOR_INFO
                    ),
                    ephemeral=True,
                )
                return
            ...
            for b in backups:
                ...
                fields.append(
                    (
                        f"`{b.get('backup_id')}`{pw_icon} – {b.get('label', 'Ohne Label')}",
                        f"Erstellt {ts_text} von <@{b.get('created_by')}>",
                        False,
                    )
                )
            await interaction.followup.send(
                embed=create_embed(
                    f"{E.CHANNEL} Backups – {interaction.guild.name}",
            ...
            await interaction.response.send_message(
                embed=create_embed(
                    "🔍 Backup-Info",
                    "Nutze `/backup_info <backup_id>` für Details.",
                    COLOR_INFO,
                ),
                ephemeral=True,
            )
            ...
            await interaction.response.send_message(
                embed=create_embed(
                    f"{E.REFRESH}️ Backup Restore",
                    "Nutze `/backup_restore <backup_id>` um ein Backup wiederherzustellen.",
                    COLOR_INFO,
                ),
                ephemeral=True,
            )
            ...
            await interaction.response.send_message(
                embed=create_embed(
                    ...
                    "Nutze `/backup_restore_cross <backup_id>` um ein Backup von einem anderen Server zu laden.",
                    COLOR_INFO,
                ),
                ephemeral=True,
            )
            ...
            await interaction.followup.send(
                embed=create_embed(
                    f"{E.DELETE} Alle Backups gelöscht",
                    f"**{result.deleted_count}** Backups gelöscht.",
                    COLOR_SUCCESS,
                ),
                ephemeral=True,
            )
            ...
            await interaction.response.send_message(
                embed=create_embed(
                    f"{E.GEAR}️ Auto-Backup",
                    "Nutze `/backup_autosetup` für die Konfiguration.",
                    COLOR_INFO,
                ),
                ephemeral=True,
            )
...
async def slash_backup_menu(interaction: discord.Interaction) -> None:
    ...
    await interaction.response.send_message(
        embed=embed, view=BackupMainView(), ephemeral=True
    )


@bot.tree.command(
    name="backup_create", description="Erstellt ein vollständiges Backup des Servers"
)
@app_commands.describe(label="Optionales Label für das Backup")
@app_commands.default_permissions(administrator=True)
async def slash_backup_create(
    interaction: discord.Interaction, label: str = None
) -> None:
    await interaction.response.defer()
    backup_data = await _collect_backup_data(interaction.guild)
    ...
    fields = [
        ("Backup-ID", f"`{backup_id}`", True),
        ("Label", label or "Automatisch", True),
        ("Erstellt von", interaction.user.mention, True),
        ("Rollen", str(len(backup_data.get("roles", []))), True),
        ("Kanäle", str(len(backup_data.get("channels", []))), True),
        ("Kategorien", str(len(backup_data.get("categories", []))), True),
    ]
...
@bot.tree.command(
    name="backup_secure", description="Erstellt ein passwortgeschütztes Backup"
)
@app_commands.default_permissions(administrator=True)
...
async def slash_backup_list(interaction: discord.Interaction) -> None:
    ...
    if not backups:
        await interaction.response.send_message(
            embed=create_embed(
                f"{E.CHANNEL} Backups",
                "Keine Backups. Nutze `/backup_create`.",
                COLOR_INFO,
            ),
            ephemeral=True,
        )
...
async def slash_backup_info(interaction: discord.Interaction, backup_id: str) -> None:
    ...
    if not doc:
        await interaction.followup.send(
            embed=create_embed(
                f"{E.FAIL} Nicht gefunden", f"Kein Backup `{backup_id}`.", COLOR_DANGER
            )
        )
        return
    data = doc.get("data", {})
    ...
    ts_text = (
        f"<t:{int(ts.timestamp())}:F>" if isinstance(ts, datetime.datetime) else str(ts)
    )
    pw_status = "🔐 Ja" if doc.get("has_password") else "{E.FAIL} Nein"
    fields = [
        ("Backup-ID", f"`{doc.get('backup_id')}`", True),
        ("Label", doc.get("label", "Ohne Label"), True),
        ("Erstellt", ts_text, True),
        ("Von", f"<@{doc.get('created_by')}>", True),
        ("Passwortgeschützt", pw_status, True),
        ("Rollen", str(len(data.get("roles", []))), True),
        ("Kanäle", str(len(data.get("channels", []))), True),
        ("Kategorien", str(len(data.get("categories", []))), True),
        ("Emojis", str(len(data.get("emojis", []))), True),
    ]
    ...
    await interaction.followup.send(
        embed=create_embed(
            f"{E.SHIELD} Backup-Info", f"Details zu **{backup_id}**", COLOR_INFO, fields
        )
    )


@bot.tree.command(
    name="backup_restore", description="Stellt ein Backup auf diesem Server wieder her"
)
@app_commands.describe(backup_id="Die Backup-ID")
@app_commands.default_permissions(administrator=True)
async def slash_backup_restore(
    interaction: discord.Interaction, backup_id: str
) -> None:
    doc = await _backup_db_get(interaction.guild.id, backup_id)
    if not doc:
        await interaction.response.send_message(
    ...
    pw_text = (
        "\n🔐 **Dieses Backup ist passwortgeschützt!** Du musst das Passwort eingeben."
        if pw
        else ""
    )
    embed = create_embed(
        f"{E.ALERT}️ Backup-Restore bestätigen",
        f"Backup **{label}** (`{backup_id}`) wiederherstellen?\n\n> Fehlende Rollen/Kanäle werden **erstellt**\n> Bestehende werden **aktualisiert**\n> Nichts wird gelöscht{pw_text}",
        COLOR_DANGER,
    )
    await interaction.response.send_message(
        embed=embed,
        view=BackupRestoreConfirmView(
            backup_id, label, interaction.guild, interaction.user, has_password=pw
        ),
    )


@bot.tree.command(
    name="backup_restore_cross",
    description="Stellt ein passwortgeschütztes Backup von einem anderen Server wieder her",
)
@app_commands.describe(backup_id="Die Backup-ID vom anderen Server")
@app_commands.default_permissions(administrator=True)
async def slash_backup_restore_cross(
    interaction: discord.Interaction, backup_id: str
) -> None:
    ...
    if not doc:
        await interaction.response.send_message(
            embed=create_embed(
                f"{E.FAIL} Nicht gefunden",
                f"Kein Backup `{backup_id}` gefunden.",
                COLOR_DANGER,
            ),
            ephemeral=True,
        )
    ...
    if not doc.get("has_password"):
        await interaction.response.send_message(
            embed=create_embed(
                f"{E.FAIL} Kein Passwort-Schutz",
                "Cross-Server-Restore erfordert ein passwortgeschütztes Backup. Erstelle es mit `/backup_secure`.",
                COLOR_WARNING,
            ),
            ephemeral=True,
        )
    ...
    embed = create_embed(
        "🌐 Cross-Server Restore",
        f"Backup **{label}** vom Server **{source_name}** (`{backup_id}`).\n\n🔐 Gib das Passwort ein, um fortzufahren.",
        COLOR_PRIMARY,
    )
    await interaction.response.send_message(
        embed=embed,
        view=BackupPasswordPromptView(
            ...
            interaction.guild,
            interaction.user,
            source_guild_id=doc.get("guild_id"),
        ),
        ephemeral=True,
    )
...
async def slash_backup_delete(interaction: discord.Interaction, backup_id: str) -> None:
    ...
    await interaction.response.send_message(
        embed=create_embed(
            "🗑️ Backup löschen?",
            f"**{label}** (`{backup_id}`) wirklich löschen?",
            COLOR_DANGER,
        ),
        view=BackupDeleteConfirmView(
            backup_id, label, interaction.guild.id, interaction.user
        ),
        ephemeral=True,
    )
...
@bot.tree.command(
    name="backup_autosetup", description="Konfiguriert automatische Backups"
)
@app_commands.describe(
...
@app_commands.choices(
    interval_hours=[
        app_commands.Choice(name="Alle 6 Stunden", value=6),
        app_commands.Choice(name="Alle 12 Stunden", value=12),
        app_commands.Choice(name="Täglich (24h)", value=24),
        app_commands.Choice(name="Alle 2 Tage (48h)", value=48),
    ]
)
@app_commands.default_permissions(administrator=True)
async def slash_backup_autosetup(
    interaction: discord.Interaction,
    enabled: bool,
    ...
    embed = create_embed(
        f"{E.SETTINGS} Auto-Backup",
        "Automatische Backups aktualisiert.",
        COLOR_SUCCESS,
        [
            ("Status", status, True),
            ("Intervall", f"{interval_hours}h", True),
            ("Max.", str(max_backups), True),
        ],
    )
    await interaction.response.send_message(embed=embed)
    await bot.log_action(
        interaction.guild,
        f"{E.SETTINGS} Auto-Backup {status}",
        f"{interaction.user.mention}: Intervall={interval_hours}h, Max={max_backups}",
        COLOR_INFO,
        user=interaction.user,
        module="backup",
    )
...
async def slash_backup_purge(interaction: discord.Interaction) -> None:
    ...
    await bot.log_action(
        interaction.guild,
        f"{E.DELETE} Backup-Purge",
        f"{interaction.user.mention}: {result.deleted_count} Backups.",
        COLOR_WARNING,
        user=interaction.user,
        module="backup",
    )
    ...
    for guild in bot.guilds:
        try:
            ...
            if last_backup_ts:
                try:
                    last_dt = (
                        datetime.datetime.fromisoformat(last_backup_ts.replace("Z", ""))
                        if isinstance(last_backup_ts, str)
                        else last_backup_ts
                    )
                except ValueError:
            ...
            backup_id = await _backup_db_save(
                backup_data,
                bot.user.id,
                f"Auto-Backup ({now.strftime('%d.%m.%Y %H:%M')})",
            )
            if backup_id:
                ...
                try:
                    collection = bot.db.client["ModForge"]["backups"]
                    all_bk = (
                        await collection.find({"guild_id": guild.id}, {"_id": 1})
                        .sort("created_at", 1)
                        .to_list(1000)
                    )
                ...
                await bot.log_action(
                    guild,
                    f"{E.OK} Auto-Backup",
                    f"`{backup_id}` erstellt. Intervall: **{interval_hours}h**",
                    COLOR_SUCCESS,
                    module="backup",
                )
        except Exception as e:
...
async def _send_welcome(member: discord.Member) -> None:
    ...
    try:
        await channel.send(
            content=member.mention if wc.get("mention", True) else None, embed=embed
        )
    except (discord.Forbidden, discord.HTTPException):
        pass
    # DM senden
...
class WelcomeChannelSelect(discord.ui.ChannelSelect):
    ...
    async def callback(self, interaction: discord.Interaction) -> None:
        ...
        await interaction.response.edit_message(
            embed=create_embed(
                f"{E.OK} Welcome-Kanal gesetzt",
                f"Willkommens-Nachrichten werden in {channel.mention} gesendet.",
                COLOR_SUCCESS,
            ),
            view=None,
        )
...
class WelcomeSetupView(discord.ui.View):
    ...
    @discord.ui.button(
        label="Welcome-Kanal setzen", style=discord.ButtonStyle.primary, emoji="📢"
    )
    async def set_channel(
        ...
        await interaction.response.send_message(
            "Wähle den Kanal:",
            view=discord.ui.View().add_item(WelcomeChannelSelect()),
            ephemeral=True,
        )

    @discord.ui.button(
        label="Welcome aktivieren", style=discord.ButtonStyle.success, emoji=E.OK
    )
    async def enable(
        self, interaction: discord.Interaction, button: discord.ui.Button
    ) -> None:
        ...
        await interaction.response.send_message(
            embed=create_embed(
                f"{E.OK} Welcome aktiviert",
                "Willkommens-Nachrichten sind jetzt aktiv.",
                COLOR_SUCCESS,
            ),
            ephemeral=True,
        )
    ...
    @discord.ui.button(
        label="Leave-Kanal setzen", style=discord.ButtonStyle.secondary, emoji="👋"
    )
    async def set_leave_ch(
        self, interaction: discord.Interaction, button: discord.ui.Button
    ) -> None:
        await interaction.response.send_message(
            "Wähle den Leave-Kanal:",
            view=discord.ui.View().add_item(LeaveChannelSelect()),
            ephemeral=True,
        )

    @discord.ui.button(
        label="Leave aktivieren", style=discord.ButtonStyle.green, emoji="🚪"
    )
    async def enable_leave(
        self, interaction: discord.Interaction, button: discord.ui.Button
    ) -> None:
        cfg = bot.db.get_config(interaction.guild.id)
        lc = cfg.get("leave", {})
        ...
        await interaction.response.send_message(
            embed=create_embed(
                f"{E.OK} Leave aktiviert",
                "Leave-Nachrichten sind jetzt aktiv.",
                COLOR_SUCCESS,
            ),
            ephemeral=True,
        )
...
class LeaveChannelSelect(discord.ui.ChannelSelect):
    def __init__(self) -> None:
        super().__init__(
            channel_types=[discord.ChannelType.text],
            placeholder="Leave-Kanal wählen...",
            max_values=1,
        )
    ...
    async def callback(self, interaction: discord.Interaction) -> None:
        ...
        await interaction.response.edit_message(
            embed=create_embed(
                f"{E.OK} Leave-Kanal gesetzt",
                f"Leave-Nachrichten in {channel.mention}.",
                COLOR_SUCCESS,
            ),
            view=None,
        )
...
@bot.tree.command(
    name="welcome_setup", description="Richtet das Welcome & Leave System ein"
)
@app_commands.default_permissions(administrator=True)
async def slash_welcome_setup(interaction: discord.Interaction) -> None:
    embed = create_embed(
        "👋 Welcome & Leave Setup",
        "Konfiguriere Welcome- und Leave-Nachrichten.\n\nVariablen: `{mention}` `{user}` `{server}` `{count}` `{name}` `{id}`\n\nNutze das Web-Dashboard für den Embed-Builder mit Live-Vorschau.",
        COLOR_PRIMARY,
    )
    await interaction.response.send_message(
        embed=embed, view=WelcomeSetupView(), ephemeral=True
    )


@bot.tree.command(
    name="welcome_channel", description="Setzt den Kanal für Willkommens-Nachrichten"
)
@app_commands.describe(channel="Der Welcome-Kanal")
@app_commands.default_permissions(administrator=True)
async def slash_welcome_channel(
    interaction: discord.Interaction, channel: discord.TextChannel
) -> None:
    cfg = bot.db.get_config(interaction.guild.id)
    wc = cfg.get("welcome", {})
    ...
    await interaction.response.send_message(
        embed=create_embed(
            f"{E.OK} Welcome-Kanal",
            f"Willkommens-Nachrichten in {channel.mention}.",
            COLOR_SUCCESS,
        )
    )
...
@bot.tree.command(
    name="leave_channel", description="Setzt den Kanal für Leave-Nachrichten"
)
@app_commands.describe(channel="Der Leave-Kanal")
@app_commands.default_permissions(administrator=True)
async def slash_leave_channel(
    interaction: discord.Interaction, channel: discord.TextChannel
) -> None:
    cfg = bot.db.get_config(interaction.guild.id)
    lc = cfg.get("leave", {})
...
class AutoRoleDropdown(discord.ui.Select):
    def __init__(self, roles: list) -> None:
        options = [
            discord.SelectOption(label=r.name, value=str(r.id), emoji=E.LABEL)
            for r in roles[:25]
        ]
        super().__init__(
            placeholder="Rolle(n) für Auto-Role wählen...",
            options=options,
            min_values=1,
    ...
    async def callback(self, interaction: discord.Interaction) -> None:
        ...
        await interaction.response.edit_message(
            embed=create_embed(
                f"{E.OK} Auto-Rollen gesetzt",
                f"Neue Mitglieder erhalten: {role_mentions}",
                COLOR_SUCCESS,
            ),
            view=None,
        )
...
@bot.tree.command(
    name="autorole", description="Setzt Rollen die neue Mitglieder automatisch erhalten"
)
@app_commands.describe(
    role="Rolle die automatisch gegeben wird (oder nutze Dropdown ohne Parameter)"
)
@app_commands.default_permissions(administrator=True)
async def slash_autorole(
    ...
    if role:
        ...
        await interaction.response.send_message(
            embed=create_embed(
                f"{E.OK} Auto-Role",
                f"{role.mention} wird neuen Mitgliedern gegeben.",
                COLOR_SUCCESS,
            )
        )
    else:
        ...
        if not manageable_roles:
            await interaction.response.send_message(
                embed=create_embed(
                    f"{E.FAIL} Keine Rollen",
                    "Keine verwaltbaren Rollen gefunden.",
                    COLOR_DANGER,
                ),
                ephemeral=True,
            )
        ...
        await interaction.response.send_message(
            embed=create_embed(
                f"{E.LABEL} Auto-Role wählen",
                "Wähle eine oder mehrere Rollen:",
                COLOR_PRIMARY,
            ),
            view=AutoRoleView(manageable_roles),
            ephemeral=True,
        )
...
@app_commands.describe(role="Die zu entfernende Rolle")
@app_commands.default_permissions(administrator=True)
async def slash_autorole_remove(
    interaction: discord.Interaction, role: discord.Role
) -> None:
    cfg = bot.db.get_config(interaction.guild.id)
    wc = cfg.get("welcome", {})
    ...
    if role.id in roles_list:
        ...
        await interaction.response.send_message(
            embed=create_embed(
                f"{E.OK} Entfernt",
                f"{role.mention} ist keine Auto-Role mehr.",
                COLOR_SUCCESS,
            )
        )
    else:
        await interaction.response.send_message(
            embed=create_embed(
                f"{E.FAIL} Nicht gefunden",
                f"{role.mention} war keine Auto-Role.",
                COLOR_WARNING,
            ),
            ephemeral=True,
        )
...
class CasesPageView(discord.ui.View):
    def __init__(
        self, cases: list, guild: discord.Guild, page: int = 0, per_page: int = 10
    ) -> None:
        super().__init__(timeout=120)
        self.cases = cases
        self.guild = guild
    ...
    def _get_embed(self) -> discord.Embed:
        ...
        if not page_cases:
            return create_embed(
                f"{E.CHANNEL} Cases", "Keine Cases vorhanden.", COLOR_INFO
            )
        fields = []
        for c in page_cases:
            ts = c.get("created_at")
            ...
            fields.append(
                (
                    f"Case #{c.get('case_id', '?')} – {c.get('action', '?').upper()}",
                    f"<@{c.get('user_id', '?')}> | {c.get('reason', '—')[:60]} | {ts_text}",
                    False,
                )
            )
        return create_embed(
            f"{E.CHANNEL} Cases – {self.guild.name} (Seite {self.page + 1}/{self.max_page + 1})",
            f"**{len(self.cases)}** Cases insgesamt.",
            COLOR_INFO,
            fields,
        )
...
async def slash_cases(
    ...
    if not cases:
        await interaction.followup.send(
            embed=create_embed(
                f"{E.CHANNEL} Cases", "Keine Cases gefunden.", COLOR_INFO
            ),
            ephemeral=True,
        )
        return
...
async def _save_sticky_roles(member: discord.Member) -> None:
    ...
    member_sticky = [
        ...
        if r.id in sticky_roles and not r.is_default() and not r.managed
    ]
    if member_sticky:
        try:
            ...
            await collection.update_one(
                ...
                {"$set": {"roles": member_sticky, "saved_at": discord.utils.utcnow()}},
                upsert=True,
            )
        except Exception as e:
...
class StickyRoleDropdown(discord.ui.Select):
    def __init__(self, roles: list, current_sticky: list) -> None:
        options = [
            discord.SelectOption(
                label=r.name,
                value=str(r.id),
                default=r.id in current_sticky,
                emoji="📌",
            )
            for r in roles[:25]
        ]
        super().__init__(
            placeholder="Sticky-Rollen wählen...",
            options=options,
            min_values=0,
            max_values=min(len(options), 10),
        )
    ...
    async def callback(self, interaction: discord.Interaction) -> None:
        ...
        await bot.db.set_config(interaction.guild.id, cfg)
        roles_text = (
            " ".join(f"<@&{v}>" for v in self.values) if self.values else "Keine"
        )
        await interaction.response.edit_message(
            embed=create_embed(
                f"{E.OK} Sticky-Rollen aktualisiert",
                f"Aktive Sticky-Rollen: {roles_text}",
                COLOR_SUCCESS,
            ),
            view=None,
        )
...
@bot.tree.command(
    name="stickyrole",
    description="Konfiguriert Sticky-Rollen (werden nach Rejoin zurückgegeben)",
)
@app_commands.describe(
    role="Rolle als Sticky markieren (oder ohne Parameter für Dropdown)"
)
@app_commands.default_permissions(administrator=True)
async def slash_stickyrole(
    interaction: discord.Interaction, role: Optional[discord.Role] = None
) -> None:
    cfg = bot.db.get_config(interaction.guild.id)
    ...
    if role:
        ...
        await interaction.response.send_message(
            embed=create_embed(
                f"{E.OK} Sticky-Role",
                f"{role.mention} ist jetzt eine Sticky-Role.",
                COLOR_SUCCESS,
            )
        )
    else:
        ...
        if not manageable:
            await interaction.response.send_message(
                embed=create_embed(
                    f"{E.FAIL} Keine Rollen", "Keine verwaltbaren Rollen.", COLOR_DANGER
                ),
                ephemeral=True,
            )
            return
        await interaction.response.send_message(
            embed=create_embed(
                "📌 Sticky-Rollen",
                "Wähle Rollen die nach Rejoin zurückgegeben werden:",
                COLOR_PRIMARY,
            ),
            view=StickyRoleView(manageable, sticky),
            ephemeral=True,
        )
...
@app_commands.describe(role="Die zu entfernende Rolle")
@app_commands.default_permissions(administrator=True)
async def slash_stickyrole_remove(
    interaction: discord.Interaction, role: discord.Role
) -> None:
    cfg = bot.db.get_config(interaction.guild.id)
    sticky = cfg.get("sticky_roles", [])
    if role.id in sticky:
        ...
        await interaction.response.send_message(
            embed=create_embed(
                f"{E.OK} Entfernt",
                f"{role.mention} ist keine Sticky-Role mehr.",
                COLOR_SUCCESS,
            )
        )
    else:
        await interaction.response.send_message(
            embed=create_embed(
                f"{E.FAIL} Nicht gefunden",
                f"{role.mention} ist keine Sticky-Role.",
                COLOR_WARNING,
            ),
            ephemeral=True,
        )
    ...
    def __init__(self) -> None:
        options = [
            discord.SelectOption(
                ...
                value="create",
                description="Temp-Voice-Setup starten",
                emoji="🎤",
            ),
            discord.SelectOption(
                label=f"{E.GEAR}️ Einstellungen",
                value="settings",
                description="Temp-Voice konfigurieren",
                emoji="{E.GEAR}️",
            ),
            discord.SelectOption(
                label="🗑️ Setup entfernen",
                value="remove",
                description="Temp-Voice deaktivieren",
                emoji="🗑️",
            ),
        ]
        super().__init__(
            placeholder="Temp-Voice Aktion...",
            options=options,
            min_values=1,
            max_values=1,
        )
    ...
    async def callback(self, interaction: discord.Interaction) -> None:
        ...
        if val == "create":
            await interaction.response.send_message(
                embed=create_embed(
                    "🎤 Temp-Voice Setup",
                    "Nutze `/tempvoice_setup <channel>` um einen Join-to-Create Kanal einzurichten.",
                    COLOR_INFO,
                ),
                ephemeral=True,
            )
            ...
            fields = [
                ("Aktiviert", f"{'{E.OK}' if tv.get('enabled') else '{E.FAIL}'}", True),
                (
                    "Join-Kanal",
                    f"<#{tv.get('channel_id')}>"
                    if tv.get("channel_id")
                    else "Nicht gesetzt",
                    True,
                ),
                (
                    "Kategorie",
                    f"<#{tv.get('category_id')}>" if tv.get("category_id") else "Auto",
                    True,
                ),
            ]
            await interaction.response.edit_message(
                embed=create_embed(
                    f"{E.GEAR}️ Temp-Voice Einstellungen", "", COLOR_INFO, fields
                ),
                view=None,
            )
        elif val == "remove":
            ...
            await interaction.response.edit_message(
                embed=create_embed(
                    f"{E.OK} Temp-Voice deaktiviert",
                    "Temporäre Voice-Kanäle sind jetzt deaktiviert.",
                    COLOR_SUCCESS,
                ),
                view=None,
            )
...
async def slash_tempvoice_menu(interaction: discord.Interaction) -> None:
    await interaction.response.send_message(
        embed=create_embed("🎤 Temp-Voice System", "Wähle eine Aktion:", COLOR_PRIMARY),
...
@bot.tree.command(
    name="tempvoice_setup", description="Richtet temporäre Voice-Kanäle ein"
)
@app_commands.describe(
    channel="Der Join-to-Create Kanal", category="Kategorie für temporäre Kanäle"
)
@app_commands.default_permissions(administrator=True)
async def slash_tempvoice_setup(
    ...
    embed = create_embed(
        ...
        f"Join-to-Create: {channel.mention}\nKategorie: {category.mention if category else 'Automatisch'}",
        COLOR_SUCCESS,
    )
    await interaction.response.send_message(embed=embed)
    await bot.log_action(
        interaction.guild,
        f"{E.OK} Temp-Voice Setup",
        f"{channel.mention} als Join-to-Create von {interaction.user.mention}.",
        COLOR_SUCCESS,
        user=interaction.user,
        module="moderation",
    )
...
class MassBanConfirmView(discord.ui.View):
    ...
    @discord.ui.button(
        label="Ja, alle Bannen", style=discord.ButtonStyle.danger, emoji="🔨"
    )
    async def confirm(
        self, interaction: discord.Interaction, button: discord.ui.Button
    ) -> None:
        ...
        for uid in self.user_ids:
            try:
                ...
                await target.ban(
                    reason=f"Mass-Ban: {self.reason} (durch {interaction.user})"
                )
                banned += 1
            except (discord.NotFound, discord.Forbidden, discord.HTTPException):
                failed += 1
        ...
        embed = create_embed(
            f"{E.BAN} Mass-Ban abgeschlossen",
            f"**{banned}** gebannt, **{failed}** fehlgeschlagen.",
            COLOR_DANGER if banned > 0 else COLOR_WARNING,
        )
        await interaction.followup.send(embed=embed)
        await bot.log_action(
            interaction.guild,
            f"{E.BAN} Mass-Ban",
            f"{interaction.user.mention} hat {banned} Nutzer gebannt: {self.reason}",
            COLOR_DANGER,
            user=interaction.user,
            module="moderation",
        )
    ...
    async def cancel(
        ...
        await interaction.response.edit_message(
            embed=create_embed(
                "🚫 Abgebrochen", "Mass-Ban nicht ausgeführt.", COLOR_WARNING
            ),
            view=None,
        )
...
@app_commands.describe(
    user_ids="Komma-getrennte User-IDs (z.B. 123,456,789)",
    reason="Grund für den Mass-Ban",
)
@app_commands.default_permissions(ban_members=True)
async def slash_massban(
    interaction: discord.Interaction, user_ids: str, reason: str = "Mass-Ban"
) -> None:
    id_list = []
    ...
    if not id_list:
        await interaction.response.send_message(
            embed=create_embed(
                f"{E.FAIL} Fehler", "Keine gültigen IDs gefunden.", COLOR_DANGER
            ),
            ephemeral=True,
        )
        return
    if len(id_list) > 50:
        await interaction.response.send_message(
            embed=create_embed(
                f"{E.FAIL} Zu viele", "Max. 50 Nutzer pro Mass-Ban.", COLOR_DANGER
            ),
            ephemeral=True,
        )
        return
    ...
    embed = create_embed(
        f"{E.ALERT}️ Mass-Ban bestätigen",
        f"**{len(id_list)} Nutzer** werden gebannt.\nGrund: {reason}\nIDs: {id_text}",
        COLOR_DANGER,
    )
    await interaction.response.send_message(
        embed=embed, view=MassBanConfirmView(id_list, reason, interaction.user)
    )
...
class ReactionRoleDropdown(discord.ui.Select):
    ...
    async def callback(self, interaction: discord.Interaction) -> None:
        ...
        await interaction.response.edit_message(
            embed=create_embed(
                f"{E.OK} Rollen erhalten",
                f"Dir wurden folgende Rollen gegeben: {role_mentions}",
                COLOR_SUCCESS,
            ),
            view=None,
        )
        for r_id in selected_roles:
            ...
            if role:
                try:
                    await interaction.user.add_roles(
                        role, reason="Reaction Role (Dropdown)"
                    )
                except (discord.Forbidden, discord.HTTPException):
                    pass
...
class ReactionRoleView(discord.ui.View):
    ...
    @discord.ui.select(
        ...
        min_values=1,
        max_values=1,
        custom_id="modforge:reactionrole_select",
    )
    async def select_role(
        ...
        if role.managed or role.is_default():
            await interaction.response.send_message(
                f"{E.FAIL} Diese Rolle kann nicht vergeben werden.", ephemeral=True
            )
            return
        try:
            if role in interaction.user.roles:
                await interaction.user.remove_roles(
                    role, reason="Reaction Role (abgewählt)"
                )
                await interaction.response.send_message(
                    embed=create_embed(
                        f"{E.OK} Rolle entfernt",
                        f"{role.mention} wurde entfernt.",
                        COLOR_WARNING,
                    ),
                    ephemeral=True,
                )
                ...
                await interaction.response.send_message(
                    embed=create_embed(
                        f"{E.OK} Rolle erhalten",
                        f"{role.mention} wurde dir gegeben.",
                        COLOR_SUCCESS,
                    ),
                    ephemeral=True,
                )
            ...
            await interaction.response.send_message(
                f"{E.FAIL} Keine Berechtigung.", ephemeral=True
            )


@bot.tree.command(
    name="reactionrole",
    description="Erstellt eine Reaction-Role Nachricht mit Dropdown",
)
@app_commands.describe(
    channel="Kanal für die Reaction-Role Nachricht",
...
async def slash_reactionrole(
    ...
    description: str = "Klicke auf das Dropdown um Rollen zu erhalten oder zu entfernen.",
) -> None:
    embed = create_embed(
        title,
        description,
        COLOR_PRIMARY,
        [("Anleitung", "Nutze das Dropdown unten um Rollen zu togglen.", False)],
        thumbnail=interaction.guild.icon.url if interaction.guild.icon else None,
    )
    ...
    try:
        ...
        await interaction.response.send_message(
            embed=create_embed(
                f"{E.OK} Reaction-Role erstellt",
                f"Nachricht in {channel.mention}: [Link]({msg.jump_url})",
                COLOR_SUCCESS,
            )
        )
        await bot.log_action(
            interaction.guild,
            f"{E.OK} Reaction-Role",
            f"Dropdown in {channel.mention} von {interaction.user.mention}.",
            COLOR_SUCCESS,
            user=interaction.user,
            module="moderation",
        )
        ...
        await interaction.response.send_message(
            embed=create_embed(
                f"{E.FAIL} Keine Rechte",
                "Ich darf dort keine Nachrichten senden.",
                COLOR_DANGER,
            ),
            ephemeral=True,
        )
...
async def _userinfo_logic(guild, target, requester):
    ...
    try:
        case_count = (
            await bot.db.cases.count_documents(
    ...
    if target.voice and target.voice.channel:
        v = target.voice
        flags = []
        if v.mute or v.self_mute:
            flags.append("🔇 Muted")
        if v.deaf or v.self_deaf:
...
async def _serverinfo_logic(guild):
    ...
    fields = [
        ...
        (
            "{E.FOLDER} Kanäle",
            f"{E.TEXT} {text_ch} Text · {E.VOICE} {voice_ch} Voice · {E.CATEGORY} {cats} Kategorien",
            False,
        ),
        ...
        (
            "🛡️ ModForge",
            f"{active_mods}/6 Module aktiv · Security Level {sec_level}",
            False,
        ),
    ]
    embed = create_embed(
...
async def _modstats_logic(guild, moderator=None):
    ...
    if moderator:
        ...
        mutes = sum(
            1 for c in cases_raw if c.get("action") in ("timeout", "mute", "tempmute")
        )
        total = len(cases_raw)
        last_case = cases_raw[0] if cases_raw else None
        last_str = (
            f"<t:{int(last_case['timestamp'].timestamp())}:R>"
            if last_case and last_case.get("timestamp")
            else "Keine"
        )
...
async def _softban_logic(guild, moderator, target, reason, days=1):
    if not can_moderate(moderator, target):
        return create_embed(
            f"{E.FAIL} Fehler", "Du kannst diesen User nicht bestrafen.", COLOR_DANGER
        )
    try:
        await target.ban(
            reason=f"Softban: {reason} (durch {moderator})",
            delete_message_seconds=days * 86400,
        )
        await guild.unban(target, reason="Softban (auto-unban)")
        case_id = await bot.db.acreate_case(
            guild.id, target.id, moderator.id, "softban", reason
        )
        await bot.log_action(
            guild,
            "🔨 Softban",
            f"{target.mention} wurde von {moderator.mention} softgebannt.\nGrund: {reason}\nNachrichten: {days} Tag(e) gelöscht",
            COLOR_DANGER,
            user=target,
            module="moderation",
        )
        return create_embed(
            f"{E.OK} Softban",
            f"{target.mention} wurde softgebannt.\nGrund: {reason}\nCase: #{case_id}",
            COLOR_SUCCESS,
        )
    except discord.Forbidden:
...
async def slash_invites_lb(interaction: discord.Interaction) -> None:
    try:
        ...
        await interaction.response.send_message(
            embed=create_embed("📨 Invite Leaderboard", lb, COLOR_PRIMARY)
        )
    except discord.Forbidden:
        await interaction.response.send_message(
            embed=create_embed(f"{E.FAIL}", "Keine Berechtigung.", COLOR_DANGER),
            ephemeral=True,
        )
...
@app_commands.describe(
    trigger="Trigger-Wort/Phrase", response="Die Antwort die gesendet wird"
)
@app_commands.default_permissions(administrator=True)
async def slash_ar_add(
    ...
    await interaction.response.send_message(
        embed=create_embed(
            f"{E.OK} Auto-Response hinzugefügt",
            f"Trigger: `{trigger}`\nAntwort: {response}",
            COLOR_SUCCESS,
        )
    )
...
async def slash_ar_list(interaction: discord.Interaction) -> None:
    ...
    if not ars:
        return await interaction.response.send_message(
            embed=create_embed(
                "📝 Auto-Responses",
                "Keine eingerichtet. Nutze `/autoresponse_add`.",
                COLOR_INFO,
            )
        )
    listing = ""
    ...
    await interaction.response.send_message(
        embed=create_embed("📝 Auto-Responses", listing, COLOR_INFO)
    )


@bot.tree.command(
    name="autoresponse_del",
    description="Löscht eine Auto-Response (Index aus /autoresponse_list)",
)
...
async def slash_ar_del(interaction: discord.Interaction, index: int) -> None:
    ...
    if index < 1 or index > len(ars):
        return await interaction.response.send_message(
            embed=create_embed(
                f"{E.FAIL}",
                "Ungültiger Index. Nutze `/autoresponse_list`.",
                COLOR_DANGER,
            ),
            ephemeral=True,
        )
    ...
    await interaction.response.send_message(
        embed=create_embed(
            f"{E.OK} Gelöscht",
            f"Trigger `{removed['trigger']}` entfernt.",
            COLOR_SUCCESS,
        )
    )
...
# ═══════════════════════════════════════════════════════════════════
@bot.tree.command(
    name="noprefix_add", description="Fügt einen User zur No-Prefix-Whitelist hinzu"
)
@app_commands.describe(member="User der No-Prefix nutzen darf")
@app_commands.default_permissions(administrator=True)
async def slash_noprefix_add(
    interaction: discord.Interaction, member: discord.Member
) -> None:
    cfg = bot.db.get_config(interaction.guild.id)
    ...
    await interaction.response.send_message(
        embed=create_embed(
            f"{E.OK}", f"{member.mention} kann jetzt No-Prefix nutzen.", COLOR_SUCCESS
        )
    )


@bot.tree.command(
    name="noprefix_remove",
    description="Entfernt einen User von der No-Prefix-Whitelist",
)
@app_commands.describe(member="User entfernen")
@app_commands.default_permissions(administrator=True)
async def slash_noprefix_remove(
    interaction: discord.Interaction, member: discord.Member
) -> None:
    cfg = bot.db.get_config(interaction.guild.id)
    np_users = cfg.get("no_prefix_users", [])
    ...
    await interaction.response.send_message(
        embed=create_embed(
            f"{E.OK}", f"{member.mention} von No-Prefix entfernt.", COLOR_WARNING
        )
    )
...
async def slash_noprefix_list(interaction: discord.Interaction) -> None:
    ...
    if not enabled:
        return await interaction.response.send_message(
            embed=create_embed(
                f"{E.GEAR}️ No-Prefix",
                "No-Prefix ist **deaktiviert**. `/noprefix true` zum Aktivieren.",
                COLOR_WARNING,
            )
        )
    if not np_users:
    ...
    await interaction.response.send_message(
        embed=create_embed(f"{E.GEAR}️ No-Prefix Whitelist", desc, COLOR_INFO)
    )
...
# ═══════════════════════════════════════════════════════════════════
@bot.tree.command(
    name="purge_user", description="Löscht Nachrichten eines bestimmten Users"
)
@app_commands.describe(member="User", amount="Anzahl zu prüfender Nachrichten")
@app_commands.default_permissions(manage_messages=True)
async def slash_purge_user(
    interaction: discord.Interaction, member: discord.Member, amount: int = 100
) -> None:
    await interaction.response.defer(ephemeral=True)
    ...
    await interaction.followup.send(
        embed=create_embed(
            f"{E.DELETE}",
            f"**{len(deleted)}** Nachrichten von {member.mention} gelöscht.",
            COLOR_SUCCESS,
        )
    )
    await bot.log_action(
        interaction.guild,
        f"{E.DELETE} Purge User",
        f"{interaction.user.mention} hat {len(deleted)} Nachrichten von {member.mention} gelöscht.",
        COLOR_WARNING,
        user=interaction.user,
        module="moderation",
    )
...
async def slash_purge_bots(interaction: discord.Interaction, amount: int = 100) -> None:
    await interaction.response.defer(ephemeral=True)
    deleted = await interaction.channel.purge(
        limit=min(amount, 500), check=lambda m: m.author.bot
    )
    await interaction.followup.send(
        embed=create_embed(
            f"{E.DELETE}",
            f"**{len(deleted)}** Bot-Nachrichten gelöscht.",
            COLOR_SUCCESS,
        )
    )
...
@app_commands.describe(amount="Anzahl")
@app_commands.default_permissions(manage_messages=True)
async def slash_purge_links(
    interaction: discord.Interaction, amount: int = 100
) -> None:
    await interaction.response.defer(ephemeral=True)
    deleted = await interaction.channel.purge(
        limit=min(amount, 500), check=lambda m: "http" in m.content.lower()
    )
    await interaction.followup.send(
        embed=create_embed(
            f"{E.DELETE}",
            f"**{len(deleted)}** Link-Nachrichten gelöscht.",
            COLOR_SUCCESS,
        )
    )
...
@bot.tree.command(
    name="purge_images", description="Löscht nur Nachrichten mit Bildern/Dateien"
)
@app_commands.describe(amount="Anzahl")
@app_commands.default_permissions(manage_messages=True)
async def slash_purge_images(
    interaction: discord.Interaction, amount: int = 100
) -> None:
    await interaction.response.defer(ephemeral=True)
    deleted = await interaction.channel.purge(
        limit=min(amount, 500), check=lambda m: len(m.attachments) > 0
    )
    await interaction.followup.send(
        embed=create_embed(
            f"{E.DELETE}",
            f"**{len(deleted)}** Bild/Datei-Nachrichten gelöscht.",
            COLOR_SUCCESS,
        )
    )
...
async def prefix_purge(ctx, target: str = None, amount: int = 50):
    ...
    if target == "bots":
        ...
        def check_fn(m):
            return "http" in m.content.lower()
    ...
    msg = await ctx.send(
        embed=create_embed(
            f"{E.DELETE}", f"**{len(deleted)}** {label} gelöscht.", COLOR_SUCCESS
        )
    )
    await asyncio.sleep(5)
    try:
...
async def _userinfo(guild, target, requester):
    ...
    if target.voice and target.voice.channel:
        ...
        voice_str = f"{v.channel.mention}" + (f" ({' '.join(flags)})" if flags else "")
    e = create_embed(
        f"{E.USERS} {target.display_name}",
        f"{target.mention} (`{target.id}`)",
        COLOR_INFO,
        [
            ("📅 Erstellt", created, True),
            ("📥 Beigetreten", joined, True),
            (f"{rc} Risk", f"{risk}/100", True),
            ("⏳ Timeout", timeout_str, True),
            ("🎤 Voice", voice_str, True),
...
async def slash_userinfo(
    ...
    try:
        await interaction.response.send_message(
            embed=await _userinfo(
                interaction.guild, member or interaction.user, interaction.user
            )
        )
    except Exception as e:
...
async def prefix_userinfo(ctx, member: discord.Member = None):
    try:
        await ctx.send(
            embed=await _userinfo(ctx.guild, member or ctx.author, ctx.author)
        )
    except Exception as e:
    ...
    try:
        ...
        e = create_embed(
            ...
            [
                ...
                ("👥 Members", f"{g.member_count - bots} + {bots} Bots", True),
                (
                    "💎 Boosts",
                    f"{g.premium_subscription_count or 0} (T{g.premium_tier})",
                    True,
                ),
                (f"{E.FOLDER} Kanäle", str(len(g.channels)), True),
                ("{E.LABEL} Rollen", str(len(g.roles) - 1), True),
            ],
...
async def prefix_serverinfo(ctx):
    try:
        ...
        await ctx.send(
            embed=create_embed(
                f"{E.SERVER} {g.name}",
                f"Members: {g.member_count} · Kanäle: {len(g.channels)} · Boosts: {g.premium_subscription_count or 0}",
                COLOR_PRIMARY,
                thumbnail=g.icon.url if g.icon else None,
            )
        )
...
async def slash_modstats(
    interaction: discord.Interaction, moderator: discord.Member = None
):
    try:
        await interaction.response.defer()
        cases = (
            await bot.db.cases.find(
                {"guild_id": str(interaction.guild.id)}
                | ({"moderator_id": str(moderator.id)} if moderator else {})
            )
            .sort("timestamp", -1)
            .to_list(500)
            or []
        ...
        if moderator:
            ...
            await interaction.followup.send(
                embed=create_embed(
                    ...
                    f"🔨 Bans: {bans} · 👢 Kicks: {kicks} · {E.ALERT}️ Warns: {warns} · 📋 Total: {len(cases)}",
                    COLOR_INFO,
                    thumbnail=moderator.display_avatar.url,
                )
            )
            ...
            counts = _dd(int)
            for c in cases:
                counts[c.get("moderator_id", "?")] += 1
            top = sorted(counts.items(), key=lambda x: x[1], reverse=True)[:10]
            ranking = (
                "\n".join(
                    f"{'🥇🥈🥉'[i] if i < 3 else f'`{i + 1}.`'} <@{mid}> — **{cnt}**"
                    for i, (mid, cnt) in enumerate(top)
                )
                or "Keine Daten."
            )
            await interaction.followup.send(
                embed=create_embed(
                    f"📊 Mod-Ranking ({len(cases)} Cases)", ranking, COLOR_PRIMARY
                )
            )
    except Exception as e:
        try:
    ...
    try:
        cases = (
            await bot.db.cases.find(
                ...
                | ({"moderator_id": str(member.id)} if member else {})
            ).to_list(200)
            or []
        )
        await ctx.send(
            embed=create_embed("📊 Mod-Stats", f"Total: {len(cases)} Cases", COLOR_INFO)
        )
    except Exception as e:
        await ctx.send(f"Fehler: {e}")
...
async def slash_softban(
    ...
    try:
        ...
        dm_e = create_embed(
            f"{E.BAN} Softban",
            f"Du wurdest von **{interaction.guild.name}** softgebannt.\n**Grund:** {reason}",
            COLOR_DANGER,
            thumbnail=interaction.guild.icon.url if interaction.guild.icon else None,
        )
        await safe_dm(
            member, dm_e, cooldown_key=f"softban:{interaction.guild.id}:{member.id}"
        )
        await member.ban(
            reason=f"Softban: {reason}", delete_message_seconds=days * 86400
        )
        await interaction.guild.unban(member, reason="Softban auto-unban")
        case_id = await bot.db.acreate_case(
            interaction.guild.id, member.id, interaction.user.id, "softban", reason
        )
        await bot.log_action(
            interaction.guild,
            "🔨 Softban",
            f"{member.mention} von {interaction.user.mention}\nGrund: {reason}",
            COLOR_DANGER,
            user=member,
            module="moderation",
        )
        await interaction.response.send_message(
            embed=create_embed(
                f"{E.OK} Softban",
                f"{member.mention} softgebannt. Case #{case_id}",
                COLOR_SUCCESS,
            )
        )
    except Exception as e:
...
async def slash_report(
    ...
    try:
        ...
        if not ch_id:
            return await interaction.response.send_message(
                "Kein Report-Kanal.", ephemeral=True
            )
        ...
        await interaction.response.send_message(
            embed=create_embed(f"{E.OK}", "Report gesendet.", COLOR_SUCCESS),
            ephemeral=True,
        )
    except Exception as e:
        try:
...
async def slash_report_setup(
    ...
    try:
        ...
        await interaction.response.send_message(
            embed=create_embed(
                f"{E.OK}", f"Report-Kanal: {channel.mention}", COLOR_SUCCESS
            )
        )
    except Exception as e:
        try:
...
async def prefix_report(ctx, member: discord.Member = None, *, reason="Kein Grund"):
    if not member:
        return await ctx.send("Nutze: `!report @User Grund`")
    try:
        cfg = bot.db.get_config(ctx.guild.id)
        ch_id = cfg.get("report_channel") or cfg.get("log_channel")
        if ch_id:
...
async def slash_invites(
    ...
    try:
        ...
        for inv in sorted(user_invites, key=lambda x: x.uses, reverse=True)[:10]:
            created = (
                f"<t:{int(inv.created_at.timestamp())}:R>" if inv.created_at else "?"
            )
            expires = ""
            if inv.max_age:
                exp_ts = (
                    int(inv.created_at.timestamp() + inv.max_age)
                    if inv.created_at
                    else 0
                )
                if exp_ts > time.time():
        ...
        embed.set_footer(
            text=f"Gesamt: {total_uses} Einladungen · {interaction.guild.name}"
        )
        await interaction.response.send_message(embed=embed)
    except discord.Forbidden:
        await interaction.response.send_message(
            embed=create_embed(
                f"{E.FAIL}", "Keine Berechtigung für Invite-Zugriff.", COLOR_DANGER
            ),
            ephemeral=True,
        )
    except Exception as e:
...
async def slash_invite_track(interaction: discord.Interaction, enabled: bool):
    try:
        ...
        await interaction.response.send_message(
            embed=create_embed(
                f"{E.OK}",
                f"Invite-Tracking {'aktiviert' if enabled else 'deaktiviert'}",
                COLOR_SUCCESS if enabled else COLOR_WARNING,
            )
        )
    except Exception as e:
...
async def prefix_invites(ctx, member: discord.Member = None):
    try:
        ...
        await ctx.send(
            embed=create_embed(
                f"📨 {target.display_name}", f"**{total}** Einladungen", COLOR_INFO
            )
        )
    except Exception as e:
        await ctx.send(f"Fehler: {e}")
...
async def slash_lockall(interaction: discord.Interaction, reason: str = "Lockdown"):
    try:
        ...
        await bot.log_action(
            interaction.guild,
            f"{E.LOCK} Lockdown",
            f"{interaction.user.mention}: {locked} Kanäle gesperrt\nGrund: {reason}",
            COLOR_DANGER,
            user=interaction.user,
            module="moderation",
        )
        await interaction.followup.send(
            embed=create_embed(
                f"{E.LOCK}", f"**{locked}** Kanäle gesperrt.", COLOR_DANGER
            )
        )
    except Exception as e:
        try:
...
async def slash_unlockall(interaction: discord.Interaction):
    try:
        ...
        await interaction.followup.send(
            embed=create_embed(
                f"{E.UNLOCK}", f"**{ul}** Kanäle entsperrt.", COLOR_SUCCESS
            )
        )
    except Exception as e:
        try:
...
async def prefix_lockall(ctx, *, reason="Lockdown"):
    try:
        ...
        for ch in ctx.guild.text_channels:
            try:
                await ch.set_permissions(
                    ctx.guild.default_role, send_messages=False, reason=reason
                )
                locked += 1
            except Exception:
                pass
        await ctx.send(
            embed=create_embed(f"{E.LOCK}", f"{locked} Kanäle gesperrt.", COLOR_DANGER)
        )
    except Exception as e:
        await ctx.send(f"Fehler: {e}")
    ...
    try:
        ...
        for ch in ctx.guild.text_channels:
            try:
                ...
                pass
        await ctx.send(
            embed=create_embed(f"{E.UNLOCK}", f"{ul} Kanäle entsperrt.", COLOR_SUCCESS)
        )
    except Exception as e:
        await ctx.send(f"Fehler: {e}")
...
async def slash_history(interaction: discord.Interaction, member: discord.Member):
    try:
        ...
        try:
            ...
            for c in cases:
                ts = (
                    f"<t:{int(c['timestamp'].timestamp())}:R>"
                    if c.get("timestamp")
                    else ""
                )
                entries.append(
                    f"📋 **#{c.get('case_id', '?')}** {c.get('action', '?')} — {c.get('reason', '—')[:40]} {ts}"
                )
        except Exception:
            pass
        try:
            notes = (
                await bot.db.notes.find(
                    {"guild_id": str(interaction.guild.id), "user_id": str(member.id)}
                ).to_list(10)
                or []
            )
            for n in notes:
                entries.append(f"📝 Notiz: {n.get('text', '')[:40]}")
        except Exception:
            pass
        ...
        await interaction.followup.send(
            embed=create_embed(
                f"📜 History – {member.display_name} ({len(entries)})",
                "\n".join(entries[:20]) or "Keine Einträge.",
                COLOR_INFO,
                [(f"{rc} Risk", f"{risk}/100", True)],
                thumbnail=member.display_avatar.url,
            )
...
async def prefix_history(ctx, member: discord.Member = None):
    if not member:
        return await ctx.send("Nutze: `!history @User`")
    try:
        cases = (
            await bot.db.cases.find(
        ...
        lines = [
            f"📋 #{c.get('case_id', '?')} {c.get('action', '?')} — {c.get('reason', '—')[:40]}"
            for c in cases
        ]
        await ctx.send(
            embed=create_embed(
...
async def snipe_del(msg):
    if msg.author.bot or not msg.guild:
        return
    _snipe_cache.setdefault(msg.guild.id, {})[msg.channel.id] = {
        "content": msg.content[:1500],
        "author": msg.author,
        "ts": time.time(),
        "attachments": [a.url for a in msg.attachments][:3],
    }
...
async def slash_snipe(interaction: discord.Interaction):
    try:
        ...
        if not d or time.time() - d["ts"] > 300:
            return await interaction.response.send_message(
                "Nichts zu snipen.", ephemeral=True
            )
        ...
        try:
            await interaction.response.send_message(f"Fehler: {ex}", ephemeral=True)
        except Exception:
            pass
...
@app_commands.describe(
    frage="Frage", opt1="Option 1", opt2="Option 2", opt3="Option 3", opt4="Option 4"
)
...
async def prefix_poll(ctx, *, args=""):
    ...
    if len(parts) < 3:
        return await ctx.send("`!poll Frage | Opt1 | Opt2`")
    emojis = ["1️⃣", "2️⃣", "3️⃣", "4️⃣"]
...
async def slash_roleinfo(interaction: discord.Interaction, role: discord.Role):
    try:
        perms = (
            ", ".join(p[0].replace("_", " ").title() for p in role.permissions if p[1])[
                :200
            ]
            or "Keine"
        )
        await interaction.response.send_message(
            embed=create_embed(
                f"{E.LABEL} {role.name}",
                f"Members: {len(role.members)} · Pos: {role.position} · Farbe: {role.color}",
                role.color.value or COLOR_INFO,
...
async def slash_channelinfo(
    interaction: discord.Interaction, channel: discord.TextChannel
):
    try:
        await interaction.response.send_message(
            embed=create_embed(
                f"{E.FOLDER} #{channel.name}",
...
async def prefix_roleinfo(ctx, role: discord.Role = None):
    if not role:
        return await ctx.send("`!roleinfo @Rolle`")
    await ctx.send(
        embed=create_embed(
            f"{E.LABEL} {role.name}",
            f"Members: {len(role.members)} · Pos: {role.position}",
            role.color.value or COLOR_INFO,
        )
    )
...
async def prefix_channelinfo(ctx, channel: discord.TextChannel = None):
    ...
    await ctx.send(
        embed=create_embed(
            f"{E.FOLDER} #{ch.name}",
            f"Slowmode: {ch.slowmode_delay}s · NSFW: {ch.nsfw}",
            COLOR_INFO,
        )
    )
...
@app_commands.default_permissions(administrator=True)
async def slash_raidmode(
    interaction: discord.Interaction, enabled: bool, dauer: int = 0
):
    try:
        cfg = bot.db.get_config(interaction.guild.id)
        ...
        if enabled:
            ...
            await interaction.response.send_message(
                embed=create_embed(
                    ...
                    + (f"\nAuto-off in {dauer}m" if dauer else ""),
                    COLOR_DANGER,
                )
            )
            if dauer > 0:
            ...
            for ch in interaction.guild.text_channels:
                try:
                    ...
                    pass
            await interaction.response.send_message(
                embed=create_embed(E.OK, "Raid-Modus aus.", COLOR_SUCCESS)
            )
    except Exception as e:
        try:
            await interaction.response.send_message(f"Fehler: {e}", ephemeral=True)
...
async def prefix_raidmode(ctx, action="status"):
    ...
    if action in ("on", "true", "1"):
        ...
        await ctx.send(
            embed=create_embed(
                "🚨",
                f"Status: {'AKTIV {E.RED}' if cfg.get('raidmode_active') else f'{E.GREEN}'}",
                COLOR_INFO,
            )
        )
...
async def slash_noprefix(interaction: discord.Interaction, enabled: bool):
    try:
        ...
        await interaction.response.send_message(
            embed=create_embed(
                f"{E.OK}",
                f"No-Prefix {'aktiviert' if enabled else 'deaktiviert'}",
                COLOR_SUCCESS if enabled else COLOR_WARNING,
            )
        )
    except Exception as e:
...
async def prefix_noprefix(ctx, enabled: str = None):
    ...
    if enabled in ("on", "true"):
        cfg["no_prefix"] = True
    elif enabled in ("of", "false"):
        cfg["no_prefix"] = False
    else:
        cfg["no_prefix"] = not cfg.get("no_prefix", False)
    await bot.db.set_config(ctx.guild.id, cfg)
    await ctx.send(
        embed=create_embed(
            f"{E.OK}",
            f"No-Prefix {'AN' if cfg['no_prefix'] else 'AUS'}",
            COLOR_SUCCESS if cfg["no_prefix"] else COLOR_WARNING,
        )
    )
...
async def slash_autobanappeal(interaction: discord.Interaction, enabled: bool):
    try:
        ...
        await interaction.response.send_message(
            embed=create_embed(
                f"{E.OK}",
                f"Auto-Ban-Appeal {'aktiviert' if enabled else 'deaktiviert'}",
                COLOR_SUCCESS if enabled else COLOR_WARNING,
            )
        )
    except Exception as e:
...
async def slash_warndecay(interaction: discord.Interaction, days: int):
    try:
        ...
        await interaction.response.send_message(
            embed=create_embed(
                f"{E.OK}",
                f"Warn-Decay: {days} Tage" if days > 0 else "Warn-Decay deaktiviert.",
                COLOR_SUCCESS if days > 0 else COLOR_WARNING,
            )
        )
    except Exception as e:
        try:
...
@app_commands.default_permissions(administrator=True)
async def slash_antivpn(
    interaction: discord.Interaction, enabled: bool, action: str = "kick"
):
    try:
        cfg = bot.db.get_config(interaction.guild.id)
        cfg["anti_vpn"] = {
            "enabled": enabled,
            "action": action if action in ("kick", "ban", "log") else "kick",
        }
        await bot.db.set_config(interaction.guild.id, cfg)
        await interaction.response.send_message(
            embed=create_embed(
                f"{E.SHIELD}",
                f"Anti-VPN {'aktiviert' if enabled else 'deaktiviert'} · Aktion: {action}",
                COLOR_SUCCESS if enabled else COLOR_WARNING,
            )
        )
    except Exception as e:
...
@commands.has_permissions(ban_members=True)
async def prefix_tempban(
    ctx, member: discord.Member = None, duration: str = "1d", *, reason="Tempban"
):
    if not member:
        return await ctx.send("`!tempban @User 7d Grund`")
    try:
        dur = parse_duration(duration)
        if not dur:
            return await ctx.send("Ungültige Dauer.")
        await member.ban(reason=f"Tempban: {reason}", delete_message_seconds=86400)
        await ctx.send(
            embed=create_embed(
                f"{E.OK}", f"{member.mention} für {duration} gebannt.", COLOR_SUCCESS
            )
        )
    except Exception as e:
        await ctx.send(f"Fehler: {e}")
...
async def prefix_tempmute(
    ctx, member: discord.Member = None, duration: str = "1h", *, reason="Timeout"
):
    if not member:
        return await ctx.send("`!tempmute @User 2h Grund`")
    try:
        dur = parse_duration(duration)
        if not dur:
            return await ctx.send("Ungültige Dauer.")
        ...
        await ctx.send(
            embed=create_embed(
                f"{E.OK}", f"{member.mention} für {duration} gemutet.", COLOR_SUCCESS
            )
        )
    except Exception as e:
        await ctx.send(f"Fehler: {e}")
...
async def prefix_case(ctx, case_id: int = None):
    if not case_id:
        return await ctx.send("`!case <ID>`")
    try:
        c = await bot.db.cases.find_one(
            {"guild_id": str(ctx.guild.id), "case_id": case_id}
        )
        if not c:
            return await ctx.send("Case nicht gefunden.")
        await ctx.send(
            embed=create_embed(
                f"📋 #{case_id}",
                f"Aktion: {c.get('action')}\nUser: <@{c.get('user_id')}>\nMod: <@{c.get('moderator_id')}>\nGrund: {c.get('reason', '—')}",
                COLOR_INFO,
            )
        )
    except Exception as e:
...
async def prefix_status(ctx):
    try:
        ...
        await ctx.send(
            embed=create_embed(
                f"{E.BOT} Status",
                f"Server: {len(bot.guilds)} · Latenz: {lat}ms",
                COLOR_PRIMARY,
            )
        )
    except Exception as e:
...
async def prefix_stats(ctx):
    ...
    await ctx.send(
        embed=create_embed(
            f"{E.SERVER} {g.name}",
            f"Members: {g.member_count} · Kanäle: {len(g.channels)} · Rollen: {len(g.roles)}",
            COLOR_PRIMARY,
        )
    )
...
async def prefix_help(ctx):
    ...
    e = create_embed(
        ...
        "Nutze `/help` für die interaktive Hilfe.\n\n**Kategorien:**",
        COLOR_PRIMARY,
    )
    for key, (title, cmds) in HELP_DATA.items():
        e.add_field(
            name=title,
            value=" · ".join(c[0].split(" ")[0] for c in cmds[:5]) + " …",
            inline=False,
        )
    await ctx.send(embed=e)
...
async def prefix_panic(ctx):
    try:
        ...
        for ch in ctx.guild.text_channels:
            try:
                await ch.set_permissions(
                    ctx.guild.default_role, send_messages=False, reason="PANIC"
                )
            except Exception:
                pass
        await ctx.send(
            embed=create_embed(
                "🚨 LOCKDOWN",
                "Security Level 3. `!unlockdown` zum Aufheben.",
                COLOR_DANGER,
            )
        )
    except Exception as e:
    ...
    try:
        ...
        for ch in ctx.guild.text_channels:
            try:
                ...
                pass
        await ctx.send(
            embed=create_embed(f"{E.OK}", "Lockdown aufgehoben.", COLOR_SUCCESS)
        )
    except Exception as e:
        await ctx.send(f"Fehler: {e}")
...
async def prefix_report_setup(ctx, channel: discord.TextChannel = None):
    if not channel:
        return await ctx.send("`!report_setup #channel`")
    cfg = bot.db.get_config(ctx.guild.id)
    cfg["report_channel"] = channel.id
    await bot.db.set_config(ctx.guild.id, cfg)
    await ctx.send(
        embed=create_embed(f"{E.OK}", f"Report-Kanal: {channel.mention}", COLOR_SUCCESS)
    )
...
async def prefix_autorole(ctx, role: discord.Role = None):
    if not role:
        return await ctx.send("`!autorole @Rolle`")
    cfg = bot.db.get_config(ctx.guild.id)
    ...
    await ctx.send(
        embed=create_embed(
            f"{E.OK}", f"Auto-Role {role.mention} hinzugefügt.", COLOR_SUCCESS
        )
    )
...
async def prefix_massban(ctx, *, ids=""):
    if not ids:
        return await ctx.send("`!massban 123,456,789`")
    id_list = [int(p) for p in re.split(r"[,;\s]+", ids) if p.strip().isdigit()][:50]
    banned = 0
    for uid in id_list:
        try:
            await ctx.guild.ban(
                discord.Object(uid), reason=f"Massban durch {ctx.author}"
            )
            banned += 1
        except Exception:
            pass
    await ctx.send(
        embed=create_embed(
            f"{E.BAN}", f"{banned}/{len(id_list)} gebannt.", COLOR_DANGER
        )
    )
...
async def prefix_autobanappeal(ctx, enabled: str = None):
    ...
    on = (
        enabled in ("on", "true")
        if enabled
        else not cfg.get("auto_ban_appeal", {}).get("enabled", False)
    )
    ...
    await ctx.send(
        embed=create_embed(
            f"{E.OK}",
            f"Auto-Ban-Appeal {'AN' if on else 'AUS'}",
            COLOR_SUCCESS if on else COLOR_WARNING,
        )
    )
...
async def prefix_warndecay(ctx, days: int = 0):
    ...
    await ctx.send(
        embed=create_embed(
            f"{E.OK}",
            f"Warn-Decay: {days}d" if days > 0 else "Warn-Decay aus.",
            COLOR_SUCCESS if days > 0 else COLOR_WARNING,
        )
    )
...
async def prefix_antivpn(ctx, action="status"):
    cfg = bot.db.get_config(ctx.guild.id)
    if action in ("on", "true"):
        cfg["anti_vpn"] = {"enabled": True, "action": "kick"}
    elif action in ("of", "false"):
        cfg["anti_vpn"] = {"enabled": False}
    else:
        return await ctx.send(
            embed=create_embed(
                f"{E.SHIELD}",
                f"Anti-VPN: {'AN' if cfg.get('anti_vpn', {}).get('enabled') else 'AUS'}",
                COLOR_INFO,
            )
        )
    await bot.db.set_config(ctx.guild.id, cfg)
...
async def noprefix_handler(msg):
    ...
    np_users = cfg.get("no_prefix_users", [])
    ok = (
        (str(msg.author.id) in [str(u) for u in np_users])
        if np_users
        else msg.author.guild_permissions.manage_messages
    )
    if ok:
...
async def ar_handler(msg):
    ...
    for ar in cfg.get("auto_responses", []):
        if (
            ar.get("enabled", True)
            and ar.get("trigger", "").lower() in msg.content.lower()
        ):
            try:
                await msg.channel.send(ar["response"])
            except Exception:
                pass
            break
...
@app_commands.describe(
    enabled="An/Aus",
    reward_role="Belohnungsrolle für Tag-Nutzer",
    tag="Tag-Text im Namen (z.B. '| MyServer')",
)
@app_commands.default_permissions(administrator=True)
async def slash_servertag(
    interaction: discord.Interaction,
    enabled: bool,
    ...
    try:
        ...
        if not enabled:
                    ...
                    for m in old_role.members:
                        try:
                            await m.remove_roles(
                                old_role, reason="Server-Tag deaktiviert"
                            )
                            removed += 1
                        except Exception:
                            pass
            ...
            await interaction.response.send_message(
                embed=create_embed(
                    f"{E.OK} Server-Tag deaktiviert", msg_text, COLOR_WARNING
                )
            )
            return
        ...
        if not reward_role or not tag:
            return await interaction.response.send_message(
                embed=create_embed(
                    f"{E.FAIL}",
                    "Bitte gib Tag und Rolle an:\n`/servertag true @Rolle | MyServer`",
                    COLOR_DANGER,
                ),
                ephemeral=True,
            )
        ...
        await bot.log_action(
            interaction.guild,
            f"{E.LABEL} Server-Tag eingerichtet",
...
@commands.has_permissions(administrator=True)
async def prefix_servertag(
    ctx, action: str = "status", role: discord.Role = None, *, tag: str = ""
):
    try:
        cfg = bot.db.get_config(ctx.guild.id)
        ...
        if action.lower() in ("on", "true"):
            if not role or not tag:
                return await ctx.send(
                    embed=create_embed(
                        f"{E.FAIL}", "`!servertag on @Rolle | MeinTag`", COLOR_DANGER
                    )
                )
            ...
            await bot.db.set_config(ctx.guild.id, cfg)
            await ctx.send(
                embed=create_embed(f"{E.OK}", "Server-Tag deaktiviert.", COLOR_WARNING)
            )
        else:
            if st.get("enabled"):
                ...
                await ctx.send(
                    embed=create_embed(
                        f"{E.LABEL} Server-Tag",
                        f"**Status:** {E.OK} Aktiv\n**Tag:** `{st['tag']}`\n**Rolle:** {r.mention if r else '?'}\n"
                        f"**User mit Rolle:** {len(r.members) if r else 0}",
                        COLOR_SUCCESS,
                    )
                )
            else:
                await ctx.send(
                    embed=create_embed(
                        E.LABEL,
                        "Server-Tag ist **inaktiv**.\n`!servertag on @Rolle | MeinTag`",
                        COLOR_WARNING,
                    )
                )
    except Exception as e:
...
async def servertag_check(before: discord.Member, after: discord.Member):
    ...
    try:
        ...
        has_tag = (
            tag_lower in (after.display_name or "").lower()
            or tag_lower in (after.name or "").lower()
        )
        ...
        if should_have_role and role not in after.roles:
            ...
            await bot.log_action(
                after.guild,
                f"{E.LABEL} Server-Tag — Rolle gegeben",
            ...
            await bot.log_action(
                after.guild,
                f"{E.LABEL} Server-Tag — Rolle entzogen",
...
async def _apply_autonick_to_member(member: discord.Member, an: dict) -> bool:
    ...
    if new_nick is None:
        ...
        for rule in rules:
            ...
            s = rule.get("suffix", "")
            if member.nick and (
                (p and member.nick.startswith(p)) or (s and member.nick.endswith(s))
            ):
                try:
...
async def _bulk_apply_autonick(
    guild: discord.Guild, an: dict, role_id: str = None
) -> dict:
    """
    ...
    log.info(
        f"Bulk AutoNick [{guild.name}]: {applied} geändert, {skipped} skip, {failed} fehler / {total} gesamt"
    )
...
# ═══════════════════════════════════════════════════════════════════
@bot.tree.command(
    name="backup_share", description="Macht ein Backup öffentlich teilbar"
)
@app_commands.describe(
    backup_id="Backup-ID",
...
async def slash_backup_share(
    ...
    try:
        ...
        if not backup:
            return await interaction.response.send_message(
                embed=create_embed(f"{E.FAIL}", "Backup nicht gefunden.", COLOR_DANGER),
                ephemeral=True,
            )
        if backup.get("password_hash"):
            return await interaction.response.send_message(
                embed=create_embed(
                    f"{E.FAIL}",
                    "Passwortgeschützte Backups können nicht geteilt werden.",
                    COLOR_DANGER,
                ),
                ephemeral=True,
            )
        ...
        await col.update_one(
            {"backup_id": backup_id},
            {
                "$set": {
                    ...
                    "guild_name": interaction.guild.name,
                    "guild_icon": str(interaction.guild.icon.url)
...
@bot.tree.command(
    name="backup_unshare", description="Entfernt ein Backup aus der öffentlichen Liste"
)
@app_commands.describe(backup_id="Backup-ID")
@app_commands.default_permissions(administrator=True)
async def slash_backup_unshare(
    interaction: discord.Interaction, backup_id: str
) -> None:
    try:
        col = bot.db.client["ModForge"]["public_backups"]
        result = await col.delete_one(
            {"backup_id": backup_id, "guild_id": str(interaction.guild.id)}
        )
        if result.deleted_count:
            await interaction.response.send_message(
                embed=create_embed(
                    f"{E.OK}",
                    f"Backup `{backup_id}` ist nicht mehr öffentlich.",
                    COLOR_SUCCESS,
                )
            )
        else:
            await interaction.response.send_message(
                embed=create_embed(
                    f"{E.FAIL}", "Nicht gefunden oder nicht dein Backup.", COLOR_DANGER
                ),
                ephemeral=True,
            )
    except Exception as e:
