...
async function latestAudit(guild, type, targetId, maxAgeMs = 15000) {
  try {
    const logs = await guild.fetchAuditLogs({ type, limit: 6 });
    const now = Date.now();
    return logs.entries.find((entry) => {
      const targetOk = String(entry.target?.id || entry.targetId || '') === String(targetId);
...
const events = [
  ...
  { name: 'roleCreate', async execute(bot, role) { await bot.logAction(role.guild, '🏷️ Rolle erstellt', `${role} wurde erstellt.`, COLOR_WARNING, { module: 'roles' }); } },
  { name: 'roleDelete', async execute(bot, role) { await bot.logAction(role.guild, '🏷️ Rolle gelöscht', `**${role.name}** wurde gelöscht.`, COLOR_WARNING, { module: 'roles' }); } },
  {
