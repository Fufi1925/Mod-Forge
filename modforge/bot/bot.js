...
async function removeBlockedUserEverywhere(bot, userId) {
  const results = [];
  for (const guild of bot.guilds.cache.values()) {
    try {
      const ban = await guild.bans.fetch(String(userId)).catch(() => null);
      if (!ban) {
        results.push({ guild_id: guild.id, skipped: true, reason: 'not_banned' });
      } else {
        await guild.bans.remove(String(userId), 'ModForge Global Security Sperre entfernt');
        results.push({ guild_id: guild.id, ok: true });
      }
    } catch (error) {
      results.push({ guild_id: guild.id, ok: false, error: error.message });
    }
...
module.exports = { ModForge, Tracker, globalBanReason, notifyOwnerAboutFailedBan, enforceBlockedUserOnGuild, enforceBlockedUserEverywhere, removeBlockedUserEverywhere, enforceGlobalSecurityOnGuild, enforceAllGlobalSecurity };
